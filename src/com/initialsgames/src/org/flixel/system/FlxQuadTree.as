package org.flixel.system
{
	...
	import org.flixel.AxRect;

	/**
	 * A fairly generic quad tree structure for rapid overlap checks.
	 * AxQuadTree is also configured for single or dual list operation.
	 ...
	 */
	public class AxQuadTree extends AxRect
	{
		/**
		 ...
		 */
		protected var _headA:AxList;
		/**
		 * Refers to the internal A and B linked lists,
		 * which are used to store objects in the leaves.
		 */
		protected var _tailA:AxList;
		/**
		 * Refers to the internal A and B linked lists,
		 * which are used to store objects in the leaves.
		 */
		protected var _headB:AxList;
		/**
		 * Refers to the internal A and B linked lists,
		 * which are used to store objects in the leaves.
		 */
		protected var _tailB:AxList;
		 ...
		 * Internal, governs and assists with the formation of the tree.
		 */
		protected var _northWestTree:AxQuadTree;
		/**
		 * Internal, governs and assists with the formation of the tree.
		 */
		protected var _northEastTree:AxQuadTree;
		/**
		 * Internal, governs and assists with the formation of the tree.
		 */
		protected var _southEastTree:AxQuadTree;
		/**
		 * Internal, governs and assists with the formation of the tree.
		 */
		protected var _southWestTree:AxQuadTree;
		/**
		 * Internal, governs and assists with the formation of the tree.
		 */
		 ...
		 */
		static protected var _object:AxEntity;
		/**
		 * Internal, used to reduce recursive method parameters during object placement and tree formation.
		 */
		 ...
		 * Internal, used during tree processing and overlap checks.
		 */
		static protected var _iterator:AxList;
		 ...
		 */
		public function AxQuadTree(X:Number, Y:Number, Width:Number, Height:Number, Parent:AxQuadTree=null, offsetX:Number=0, offsetY:Number=0)
		{
			super(X,Y,Width,Height);
			_headA = _tailA = new AxList();
			_headB = _tailB = new AxList();
		 ...
		 * 
		 * @param ObjectOrGroup1	Any object that is or extends AxEntity or AxGroup.
		 * @param ObjectOrGroup2	Any object that is or extends AxEntity or AxGroup.  If null, the first parameter will be checked against itself.
		 * @param NotifyCallback	A function with the form <code>myFunction(Object1:AxEntity,Object2:AxEntity):void</code> that is called whenever two objects are found to overlap in world space, and either no ProcessCallback is specified, or the ProcessCallback returns true. 
		 * @param ProcessCallback	A function with the form <code>myFunction(Object1:AxEntity,Object2:AxEntity):Boolean</code> that is called whenever two objects are found to overlap in world space.  The NotifyCallback is only called if this function returns true.  See AxEntity.separate(). 
		 */
		public function load(ObjectOrGroup1:AxBasic, ObjectOrGroup2:AxBasic=null, NotifyCallback:Function=null, ProcessCallback:Function=null):void
		{
		 ...
		 * 
		 * @param	ObjectOrGroup	AxEntitys are just added, AxGroups are recursed and their applicable members added accordingly.
		 ...
		 */
		public function add(ObjectOrGroup:AxBasic, List:uint):void
		{
			_list = List;
			if(ObjectOrGroup is AxGroup)
			{
				...
				var l:uint = (ObjectOrGroup as AxGroup).length;
				while(i < l)
				{
					...
					if((basic != null) && basic.exists)
					{
						if(basic is AxGroup)
							add(basic,List);
						else if(basic is AxEntity)
						{
							_object = basic as AxEntity;
			...
			{
				_object = ObjectOrGroup as AxEntity;
				if(_object.exists && _object.allowCollisions)
				{
					_objectLeftEdge = _object.x;
					_objectTopEdge = _object.y;
		...
		protected function addObject():void
		{
			...
			if((_objectLeftEdge > _leftEdge) && (_objectRightEdge < _midpointX))
			{
				...
				{
					if(_northWestTree == null)
						_northWestTree = new AxQuadTree(_leftEdge,_topEdge,_halfWidth,_halfHeight,this,_offsetX,_offsetY);
				...
				{
					if(_southWestTree == null)
						_southWestTree = new AxQuadTree(_leftEdge,_midpointY,_halfWidth,_halfHeight,this,_offsetX,_offsetY);
			...
			if((_objectLeftEdge > _midpointX) && (_objectRightEdge < _rightEdge))
			{
				...
				{
					if(_northEastTree == null)
						_northEastTree = new AxQuadTree(_midpointX,_topEdge,_halfWidth,_halfHeight,this,_offsetX,_offsetY);
				...
				{
					if(_southEastTree == null)
						_southEastTree = new AxQuadTree(_midpointX,_midpointY,_halfWidth,_halfHeight,this,_offsetX,_offsetY);
		...
		/**
		 * <code>AxQuadTree</code>'s other main function.  Call this after adding objects
		 * using <code>AxQuadTree.load()</code> to compare the objects that you loaded.
		 *
