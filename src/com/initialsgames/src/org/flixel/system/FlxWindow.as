package org.flixel.system
{
	...
	/**
	 * A generic, Flash-based window class, created for use in <code>AxDebugger</code>.
	 * 
	 * @author Adam Atomic
	 */
	public class AxWindow extends Sprite
	{
		...
		/**
		 * Creates a new window object.  This Flash-based class is mainly (only?) used by <code>AxDebugger</code>.
		 * 
		 ...
		 */
		public function AxWindow(Title:String,Width:Number,Height:Number,Resizable:Boolean=true,Bounds:Rectangle=null,BGColor:uint=0x7f7f7f7f, TopColor:uint=0x7f000000)
		{
			super();
			_width = Width;
			_height = Height;
		...
		protected function bound():void
		{
			if(_bounds != null)
			{
				x = AxU.bound(x,_bounds.left,_bounds.right-_width);
				y = AxU.bound(y,_bounds.top,_bounds.bottom-_height);
			}
		}
		...
		{
			_width = AxU.bound(_width,minSize.x,maxSize.x);
			_height = AxU.bound(_height,minSize.y,maxSize.y);
