package org.flixel.system.debug
{
	...
	import org.flixel.AxG;
	import org.flixel.AxU;
	...
	public class VCR extends Sprite
	{
		...
		/**
		 * Usually called by AxGame when a requested recording has begun.
		...
		/**
		 * Usually called by AxGame when a replay has been stopped.
		 * Just updates the VCR GUI so the buttons are in the right state.
		 */
		...
		/**
		 * Usually called by AxGame when a requested replay has begun.
		 * Just updates the VCR GUI so the buttons are in the right state.
		 */
		...
		{
			_runtime += Time;
			_runtimeDisplay.text = AxU.formatTime(_runtime/1000,true);
		...
		protected function onOpenComplete(E:Event=null):void
		{
			...
			{
				Ax.log("ERROR: Empty flixel gameplay record.");
				return;
			}
			
			Ax.loadReplay(fileContents);
		}
		...
		protected function onOpenError(E:Event=null):void
		{
			...
			_file.removeEventListener(IOErrorEvent.IO_ERROR, onOpenError);
			_file = null;
			Ax.log("ERROR: Unable to open flixel gameplay record.");
		}
		 ...
		 * 
		 * @param	StandardMode	Whether to reset the whole game, or just this <code>AxState</code>.  StandardMode == false is useful for recording demos or attract modes.
		 */
		public function onRecord(StandardMode:Boolean=false):void
		{
			if(_play.visible)
				onPlay();
			Ax.recordReplay(StandardMode);
		}
		...
		{
			var data:String = Ax.stopRecording();
		...
		protected function onSaveComplete(E:Event=null):void
		{
			...
			_file = null;
			Ax.log("FLIXEL: successfully saved flixel gameplay record.");
		}
		...
		protected function onSaveError(E:Event=null):void
		{
			...
			_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
			_file = null;
			Ax.log("ERROR: problem saving flixel gameplay record.");
		}
		...
		{
			if(Ax.reloadReplay(StandardMode))
			{
				_recordOff.visible = false;
				_recordOn.visible = false;
				_stop.visible = true;
		...
		/**
		 * Just sets up basic mouse listeners, a la AxWindow.
		 * 
		 * @param	E	Flash event.
		 */
