package org.flixel
{
	...
	import org.flixel.AxBasic;
	
	/**
	 * This is the base class for most of the display objects (<code>AxSprite</code>, <code>AxText</code>, etc).
	 ...
	 * @author	Adam Atomic
	 */
	public class AxEntity extends AxBasic
	{
		/**
		 ...
		 */
		public var acceleration:AxPoint;
		/**
		 * This isn't drag exactly, more like deceleration that is only applied
		 * when acceleration is not affecting the sprite.
		 */
		public var drag:AxPoint;
		/**
		 * If you are using <code>acceleration</code>, you can use <code>maxVelocity</code> with it
		 * to cap the speed automatically (very useful!).
		 */
		public var maxVelocity:AxPoint;
		/**
		 ...
		 */
		static protected const _pZero:AxPoint = new AxPoint();
		 ...
		 */
		protected var _rect:AxRect;
		/**
		 * Set this to false if you want to skip the automatic motion/movement stuff (see <code>updateMotion()</code>).
		 * AxEntity and AxSprite default to true.
		 * AxText, AxTileblock, AxTilemap and AxSound default to false.
		 */
		...
		/**
		 * Instantiates a <code>AxEntity</code>.
		 * 
		...
		public function AxEntity(X:Number=0,Y:Number=0,Width:Number=0,Height:Number=0)
		{
			...
			velocity = new AxPoint();
			acceleration = new AxPoint();
			drag = new AxPoint();
			maxVelocity = new AxPoint(10000,10000);
			...
			scrollFactor = new AxPoint(1.0,1.0);
			_flicker = false;
			_flickerTimer = 0;
		 ...
		 * In <code>AxEntity</code> it controls the flicker timer,
		...
		override public function preUpdate():void
		{
			...
			if(_flickerTimer != 0)
			{
				if(_flickerTimer > 0)
				{
					_flickerTimer = _flickerTimer - Ax.elapsed;
					if(_flickerTimer <= 0)
					{
						_flickerTimer = 0;
						_flicker = false;
		 ...
		 * In <code>AxEntity</code> this function handles integrating the objects motion
		...
		protected function updateMotion():void
		{
			...
			velocityDelta = (AxU.computeVelocity(angularVelocity,angularAcceleration,angularDrag,maxAngular) - angularVelocity)/2;
			angularVelocity += velocityDelta; 
			angle += angularVelocity*Ax.elapsed;
			angularVelocity += velocityDelta;
			
			velocityDelta = (AxU.computeVelocity(velocity.x,acceleration.x,drag.x,maxVelocity.x) - velocity.x)/2;
			velocity.x += velocityDelta;
			delta = velocity.x*Ax.elapsed;
			velocity.x += velocityDelta;
			x += delta;
			
			velocityDelta = (AxU.computeVelocity(velocity.y,acceleration.y,drag.y,maxVelocity.y) - velocity.y)/2;
			velocity.y += velocityDelta;
			delta = velocity.y*Ax.elapsed;
			velocity.y += velocityDelta;
			y += delta;
		}
		...
		override public function drawDebug(Camera:AxCamera=null):void
		{
			...
			if(allowCollisions)
			{
				if(allowCollisions != ANY)
					boundingBoxColor = Ax.PINK;
				if(immovable)
					boundingBoxColor = Ax.GREEN;
				else
					boundingBoxColor = Ax.RED;
			}
			else
				boundingBoxColor = Ax.BLUE;
		 ...
		 * 
		 * @param	Path		The <code>AxPath</code> you want this object to follow.
		 ...
		 */
		public function followPath(Path:AxPath,Speed:Number=100,Mode:uint=PATH_FORWARD,AutoRotate:Boolean=false):void
		{
			if(Path.nodes.length <= 0)
			{
				Ax.log("WARNING: Paths need at least one node in them to be followed.");
				return;
			}
			
			path = Path;
			pathSpeed = AxU.abs(Speed);
			_pathMode = Mode;
		 ...
		 * 
		 * @return	The node (a <code>AxPoint</code> object) we are aiming for next.
		 */
		protected function advancePath(Snap:Boolean=true):AxPoint
		{
			if(Snap)
			{
				var oldNode:AxPoint = path.nodes[_pathNodeIndex];
				if(oldNode != null)
				{
		...
		protected function updatePathMotion():void
		{
			...
			var node:AxPoint = path.nodes[_pathNodeIndex];
			...
			if(horizontalOnly)
			{
				if(((deltaX>0)?deltaX:-deltaX) < pathSpeed*Ax.elapsed)
					node = advancePath();
			}
			else if(verticalOnly)
			{
				if(((deltaY>0)?deltaY:-deltaY) < pathSpeed*Ax.elapsed)
					node = advancePath();
			}
			else
			{
				if(Math.sqrt(deltaX*deltaX + deltaY*deltaY) < pathSpeed*Ax.elapsed)
					node = advancePath();
			}
			...
			if(pathSpeed != 0)
			{
				...
				{
					pathAngle = AxU.getAngle(_point,node);
					AxU.rotatePoint(0,pathSpeed,0,0,pathAngle,velocity);
				}
		...
		/**
		 * Checks to see if some <code>AxEntity</code> overlaps this <code>AxEntity</code> or <code>AxGroup</code>.
		 ...
		 */
		public function overlaps(ObjectOrGroup:AxBasic,InScreenSpace:Boolean=false,Camera:AxCamera=null):Boolean
		{
			...
			if(ObjectOrGroup is AxTilemap)
			{
				//Since tilemap's have to be the caller, not the target, to do proper tile-based collisions,
				// we redirect the call to the tilemap overlap here.
				return (ObjectOrGroup as AxTilemap).overlaps(this,InScreenSpace,Camera);
			}
			
			var object:AxEntity = ObjectOrGroup as AxEntity;
			if(!InScreenSpace)
			{
			...
			if(Camera == null)
				Camera = Ax.camera;
			var objectScreenPos:AxPoint = object.getScreenXY(null,Camera);
		 ...
		 * @return	Whether or not the two objects overlap.
		 */
		public function overlapsAt(X:Number,Y:Number,ObjectOrGroup:AxBasic,InScreenSpace:Boolean=false,Camera:AxCamera=null):Boolean
		{
			if(ObjectOrGroup is AxGroup)
			{
				var results:Boolean = false;
			...
			if(ObjectOrGroup is AxTilemap)
			{
				...
				var tilemap:AxTilemap = ObjectOrGroup as AxTilemap;
		 ...
		 */
		public function overlapsPoint(Point:AxPoint,InScreenSpace:Boolean=false,Camera:AxCamera=null):Boolean
		{
			if(!InScreenSpace)
		 ...
		 */
		public function onScreen(Camera:AxCamera=null):Boolean
		{
			if(Camera == null)
				Camera = Ax.camera;
			getScreenXY(_point,Camera);
		 ...
		 * 
		 * @param	Camera		Specify which game camera you want.  If null getScreenXY() will just grab the first global camera.
		 * @param	Point		Takes a <code>AxPoint</code> object and assigns the post-scrolled X and Y values of this object to it.
		 * 
		 * @return	The <code>Point</code> you passed in, or a new <code>Point</code> if you didn't pass one, containing the screen X and Y position of this object.
		 */
		public function getScreenXY(Point:AxPoint=null,Camera:AxCamera=null):AxPoint
		{
			if(Point == null)
				Point = new AxPoint();
			if(Camera == null)
		 ...
		 * 
		 * @Point	Allows you to pass in an existing <code>AxPoint</code> object if you're so inclined.  Otherwise a new one is created.
		 * 
		 * @return	A <code>AxPoint</code> object containing the midpoint of this object in world coordinates.
		 */
		public function getMidpoint(Point:AxPoint=null):AxPoint
		{
			if(Point == null)
				Point = new AxPoint();
			Point.x = x + width*0.5;
		 ...
		 * 
		 * @param	Object1 	Any <code>AxEntity</code>.
		 * @param	Object2		Any other <code>AxEntity</code>.
		 * 
		 * @return	Whether the objects in fact touched and were separated.
		 */
		static public function separate(Object1:AxEntity, Object2:AxEntity):Boolean
		{
		 ...
		 */
		static public function separateX(Object1:AxEntity, Object2:AxEntity):Boolean
		{
			...
			if(Object1 is AxTilemap)
				return (Object1 as AxTilemap).overlapsWithCallback(Object2,separateX);
			if(Object2 is AxTilemap)
				return (Object2 as AxTilemap).overlapsWithCallback(Object1,separateX,true);
			...
			if(obj1delta != obj2delta)
			{
				...
				var obj1rect:AxRect = new AxRect(Object1.x-((obj1delta > 0)?obj1delta:0),Object1.last.y,Object1.width+((obj1delta > 0)?obj1delta:-obj1delta),Object1.height);
				var obj2rect:AxRect = new AxRect(Object2.x-((obj2delta > 0)?obj2delta:0),Object2.last.y,Object2.width+((obj2delta > 0)?obj2delta:-obj2delta),Object2.height);
		 ...
		 */
		static public function separateY(Object1:AxEntity, Object2:AxEntity):Boolean
		{
			//can't separate two immovable objects
			var obj1immovable:Boolean = Object1.immovable;
			var obj2immovable:Boolean = Object2.immovable;
			...
			//If one of the objects is a tilemap, just pass it off.
			if(Object1 is AxTilemap)
				return (Object1 as AxTilemap).overlapsWithCallback(Object2,separateY);
			if(Object2 is AxTilemap)
				return (Object2 as AxTilemap).overlapsWithCallback(Object1,separateY,true);
			...
			if(obj1delta != obj2delta)
			{
				...
				var obj1deltaAbs:Number = (obj1delta > 0)?obj1delta:-obj1delta;
				var obj2deltaAbs:Number = (obj2delta > 0)?obj2delta:-obj2delta;
				var obj1rect:AxRect = new AxRect(Object1.x,Object1.y-((obj1delta > 0)?obj1delta:0),Object1.width,Object1.height+obj1deltaAbs);
				var obj2rect:AxRect = new AxRect(Object2.x,Object2.y-((obj2delta > 0)?obj2delta:0),Object2.width,Object2.height+obj2deltaAbs);
				if((obj1rect.x + obj1rect.width > obj2rect.x) && (obj1rect.x < obj2rect.x + obj2rect.width) && (obj1rect.y + obj1rect.height > obj2rect.y) && (obj1rect.y < obj2rect.y + obj2rect.height))
				{
					var maxOverlap:Number = obj1deltaAbs + obj2deltaAbs + OVERLAP_BIAS;
