package org.flixel
{
	...
	import org.flixel.system.AxDebugger;
	 ...
	 * Utilities for maths and color and things can be found in <code>AxU</code>.
	 * <code>AxG</code> is specifically for Flixel-specific properties.
	 * 
	 * @author	Adam Atomic
	 */
	public class AxG
	{
		 ...
		 */
		static internal var _game:AxGame;
		/**
		 ...
		 * Set automatically by <code>AxPreloader</code> during startup.
		 */
		 ...
		 */
		static public var worldBounds:AxRect;
		/**
		...
		/**
		 * <code>Ax.levels</code> and <code>Ax.scores</code> are generic
		...
		/**
		 * <code>Ax.saves</code> is a generic bucket for storing
		 * AxSaves so you can access them whenever you want.
		 */
		static public var saves:Array; 
		static public var save:int;

		/**
		 * A reference to a <code>AxMouse</code> object.  Important for input!
		 */
		static public var mouse:Mouse;
		/**
		 * A reference to a <code>AxKeyboard</code> object.  Important for input!
		 */
		static public var keys:Keyboard;
		
		/**
		 * A reference to the AxJoystick object.
		 */
		static public var joystick:AxJoystick;
		/**
		 ...
		 */
		static public var music:AxSound;
		/**
		 * A list of all the sounds being played in the game.
		 */
		static public var sounds:AxGroup;
		/**
		...
		/**
		 * An array of <code>AxCamera</code> objects that are used to draw stuff.
		 ...
		 */
		static public var camera:AxCamera;
		/**
		...
		{
			return Ax.LIBRARY_NAME + " v" + Ax.LIBRARY_MAJOR_VERSION + "." + Ax.LIBRARY_MINOR_VERSION;
		}
		...
		static public function log(Data:Object):void
		{
			if((_game != null) && (_game._debugger != null))
				_game._debugger.log.add((Data == null)?"ERROR: null object":((Data is Array)?AxU.formatArray(Data as Array):Data.toString()));
		}
		...
		{
			return globalSeed = AxU.srand(globalSeed);
		}
		
		/**
		 * Shuffles the entries in an array into a new random order.
		 * <code>Ax.shuffle()</code> is deterministic and safe for use with replays/recordings.
		 * HOWEVER, <code>AxU.shuffle()</code> is NOT deterministic and unsafe for use with replays/recordings.
		 * 
		...
		static public function shuffle(Objects:Array,HowManyTimes:uint):Array
		{
			...
			{
				index1 = Ax.random()*Objects.length;
				index2 = Ax.random()*Objects.length;
		 ...
		 * <code>Ax.getRandom()</code> is deterministic and safe for use with replays/recordings.
		 * HOWEVER, <code>AxU.getRandom()</code> is NOT deterministic and unsafe for use with replays/recordings.
		 * 
		...
		static public function getRandom(Objects:Array,StartIndex:uint=0,Length:uint=0):Object
		{
			if(Objects != null)
			{
				...
				if(l > 0)
					return Objects[StartIndex + uint(Ax.random()*l)];
			}
			return null;
		}
		 ...
		 * @param	CancelKeys	Optional parameter: an array of string names of keys (see AxKeyboard) that can be pressed to cancel the playback, e.g. ["ESCAPE","ENTER"].  Also accepts 2 custom key names: "ANY" and "MOUSE" (fairly self-explanatory I hope!).
		 * @param	Timeout		Optional parameter: set a time limit for the replay.  CancelKeys will override this if pressed.
		 * @param	Callback	Optional parameter: if set, called when the replay finishes.  Running to the end, CancelKeys, and Timeout will all trigger Callback(), but only once, and CancelKeys and Timeout will NOT call Ax.stopReplay() if Callback is set!
		 */
		static public function loadReplay(Data:String,State:AxState=null,CancelKeys:Array=null,Timeout:Number=0,Callback:Function=null):void
		{
			_game._replay.load(Data);
			if(State == null)
				Ax.resetGame();
			else
				Ax.switchState(State);
		 ...
		 * 
		 * @return	The replay data in simple ASCII format (see <code>AxReplay.save()</code>).
		 */
		...
		{
			_game._requestedState = new (AxU.getClass(AxU.getClassName(_game._state,false)))();
		}
		 ...
		 * @param	AutoDestroy		Whether to destroy this sound when it finishes playing.  Leave this value set to "false" if you want to re-use this <code>AxSound</code> instance.
		 * @param	AutoPlay		Whether to play the sound.
		 * @param	URL				Load a sound from an external web resource instead.  Only used if EmbeddedSound = null.
		 * 
		 * @return	A <code>AxSound</code> object.
		 */
		static public function loadSound(EmbeddedSound:Class=null,Volume:Number=1.0,Looped:Boolean=false,AutoDestroy:Boolean=false,AutoPlay:Boolean=false,URL:String=null):AxSound
		{
			if((EmbeddedSound == null) && (URL == null))
			{
				Ax.log("WARNING: Ax.loadSound() requires either\nan embedded sound or a URL to work.");
				return null;
			}
			var sound:AxSound = sounds.recycle(AxSound) as AxSound;
		 ...
		 * NOTE: Just calls Ax.loadSound() with AutoPlay == true.
		 * 
		 ...
		 * @return	A <code>AxSound</code> object.
		 */
		static public function play(EmbeddedSound:Class,Volume:Number=1.0,Looped:Boolean=false,AutoDestroy:Boolean=true):AxSound
		{
			return Ax.loadSound(EmbeddedSound,Volume,Looped,AutoDestroy,true);
		}
		 ...
		 * @param	AutoDestroy		Whether to destroy this sound when it finishes playing.  Leave this value set to "false" if you want to re-use this <code>AxSound</code> instance.
		 * 
		 * @return	A AxSound object.
		 */
		static public function stream(URL:String,Volume:Number=1.0,Looped:Boolean=false,AutoDestroy:Boolean=true):AxSound
		{
			return Ax.loadSound(null,Volume,Looped,AutoDestroy,true,URL);
		}
		...
		static public function set volume(Volume:Number):void
		{
			...
			if(volumeHandler != null)
				volumeHandler(Ax.mute?0:_volume);
		}

		/**
		 * Called by AxGame on state changes to stop and destroy sounds.
		 * 
		...
		static internal function destroySounds(ForceDestroy:Boolean=false):void
		{
			...
			while(i < l)
			{
				sound = sounds.members[i++] as AxSound;
		 ...
		 */
		static public function get state():AxState
		{
			return _game._state;
		}
		 ...
		 */
		static public function switchState(State:AxState):void
		{
		 ...
		 * 
		 * @return	This <code>AxCamera</code> instance.
		 */
		static public function addCamera(NewCamera:AxCamera):AxCamera
		{
			Ax._game.addChildAt(NewCamera._flashSprite,Ax._game.getChildIndex(Ax._game._mouse));
			Ax.cameras.push(NewCamera);
			return NewCamera;
		}
		 ...
		 */
		static public function removeCamera(Camera:AxCamera,Destroy:Boolean=true):void
		{
			try
			{
				Ax._game.removeChild(Camera._flashSprite);
			}
			catch(E:Error)
			{
				Ax.log("Error removing camera, not part of game.");
			}
			if(Destroy)
				Camera.destroy();
		}
		 ...
		 */
		static public function resetCameras(NewCamera:AxCamera=null):void
		{
			var cam:AxCamera;
			var i:uint = 0;
			...
			while(i < l)
			{
				cam = Ax.cameras[i++] as AxCamera;
				Ax._game.removeChild(cam._flashSprite);
				cam.destroy();
			}
			Ax.cameras.length = 0;
			
			if(NewCamera == null)
				NewCamera = new AxCamera(0,0,Ax.width,Ax.height)
			Ax.camera = Ax.addCamera(NewCamera);
		}
		...
		{
			var i:uint = 0;
			var l:uint = Ax.cameras.length;
			while(i < l)
				(Ax.cameras[i++] as AxCamera).flash(Color,Duration,OnComplete,Force);
		}
		...
		static public function fade(Color:uint=0xff000000, Duration:Number=1, OnComplete:Function=null, Force:Boolean=false):void
		{
			...
			var l:uint = Ax.cameras.length;
			while(i < l)
				(Ax.cameras[i++] as AxCamera).fade(Color,Duration,OnComplete,Force);
		}
		...
		static public function fadeStop():void
		{
			...
			var l:uint = Ax.cameras.length;
			while(i < l)
				(Ax.cameras[i++] as AxCamera).fadeStop();
		}
		...
		static public function shake(Intensity:Number=0.05, Duration:Number=0.5, OnComplete:Function=null, Force:Boolean=true, Direction:uint=0):void
		{
			...
			var l:uint = Ax.cameras.length;
			while(i < l)
				(Ax.cameras[i++] as AxCamera).shake(Intensity,Duration,OnComplete,Force,Direction);
		}
		
		/**
		 * Get and set the background color of the game.
		 * Get functionality is equivalent to Ax.camera.bgColor.
		...
		static public function set bgColor(Color:uint):void
		{
			...
			var l:uint = Ax.cameras.length;
			while(i < l)
				(Ax.cameras[i++] as AxCamera).bgColor = Color;
		}
		...
		/**
		 * Call this function to see if one <code>AxEntity</code> overlaps another.
		 * Can be called with one object and one group, or two groups, or two objects,
		 * whatever floats your boat! For maximum performance try bundling a lot of objects
		 * together using a <code>AxGroup</code> (or even bundling groups together!).
		 * 
		 ...
		 * @param	NotifyCallback	A function with two <code>AxEntity</code> parameters - e.g. <code>myOverlapFunction(Object1:AxEntity,Object2:AxEntity)</code> - that is called if those two objects overlap.
		 * @param	ProcessCallback	A function with two <code>AxEntity</code> parameters - e.g. <code>myOverlapFunction(Object1:AxEntity,Object2:AxEntity)</code> - that is called if those two objects overlap.  If a ProcessCallback is provided, then NotifyCallback will only be called if ProcessCallback returns true for those objects!
		 * 
		 * @return	Whether any oevrlaps were detected.
		 */
		static public function overlap(ObjectOrGroup1:AxBasic=null,ObjectOrGroup2:AxBasic=null,NotifyCallback:Function=null,ProcessCallback:Function=null):Boolean
		{
			if(ObjectOrGroup1 == null)
				ObjectOrGroup1 = Ax.state;
			if(ObjectOrGroup2 === ObjectOrGroup1)
				ObjectOrGroup2 = null;
			AxQuadTree.divisions = Ax.worldDivisions;
			var quadTree:AxQuadTree = new AxQuadTree(Ax.worldBounds.x,Ax.worldBounds.y,Ax.worldBounds.width,Ax.worldBounds.height);
		...
		static public function overlapWithOffset(ObjectOrGroup1:AxBasic=null,ObjectOrGroup2:AxBasic=null,offsetX:Number=0, offsetY:Number=0, NotifyCallback:Function=null,ProcessCallback:Function=null):Boolean
		{
			if(ObjectOrGroup1 == null)
				ObjectOrGroup1 = Ax.state;
			if(ObjectOrGroup2 === ObjectOrGroup1)
			...
			var quadTree:AxQuadTree = new AxQuadTree(Ax.worldBounds.x,Ax.worldBounds.y,Ax.worldBounds.width,Ax.worldBounds.height,null,offsetX,offsetY);
		...
		/**
		 * Call this function to see if one <code>AxEntity</code> collides with another.
		 * Can be called with one object and one group, or two groups, or two objects,
		 * whatever floats your boat! For maximum performance try bundling a lot of objects
		 * together using a <code>AxGroup</code> (or even bundling groups together!).
		 * 
		 * <p>This function just calls Ax.overlap and presets the ProcessCallback parameter to AxEntity.separate.
		 * To create your own collision logic, write your own ProcessCallback and use Ax.overlap to set it up.</p>
		 * 
		 * <p>NOTE: does NOT take objects' scrollfactor into account, all overlaps are checked in world space.</p>
		 * 
		 * @param	ObjectOrGroup1	The first object or group you want to check.
		 ...
		 */
		static public function collide(ObjectOrGroup1:AxBasic=null, ObjectOrGroup2:AxBasic=null, NotifyCallback:Function=null):Boolean
		{
			return overlap(ObjectOrGroup1,ObjectOrGroup2,NotifyCallback,AxEntity.separate);
		}
		
		/**
		 * Adds a new plugin to the global plugin array.
		 * 
		 * @param	Plugin	Any object that extends AxBasic. Useful for managers and other things.  See org.flixel.plugin for some examples!
		 * 
		 * @return	The same <code>AxBasic</code>-based plugin you passed in.
		 */
		static public function addPlugin(Plugin:AxBasic):AxBasic
		{
			//Don't add repeats
			var pluginList:Array = Ax.plugins;
			var i:uint = 0;
		 ...
		 * 
		 * @param	ClassType	The class name of the plugin you want to retrieve. See the <code>AxPath</code> or <code>AxTimer</code> constructors for example usage.
		 * 
		 * @return	The plugin object, or null if no matching plugin was found.
		 */
		static public function getPlugin(ClassType:Class):AxBasic
		{
			var pluginList:Array = Ax.plugins;
			var i:uint = 0;
			var l:uint = pluginList.length;
		 ...
		 * @return	The same <code>AxBasic</code>-based plugin you passed in.
		 */
		static public function removePlugin(Plugin:AxBasic):AxBasic
		{
			//Don't add repeats
			var pluginList:Array = Ax.plugins;
		...
		/**
		 * Called by <code>AxGame</code> to set up <code>AxG</code> during <code>AxGame</code>'s constructor.
		 */
		static internal function init(Game:AxGame,Width:uint,Height:uint,Zoom:Number):void
		{
			Ax._game = Game;
			Ax.width = Width;
			Ax.height = Height;
			...
			AxCamera.defaultZoom = Zoom;
			Ax._cameraRect = new Rectangle();
			Ax.cameras = new Array();
			...
			Ax.mouse = new Mouse(Ax._game._mouse);
			Ax.keys = new Keyboard();
			
			//load saved joystick prefs if they exist
			var joyPrefs:AxSave = new AxSave();
			if(joyPrefs.bind("flixel") && (joyPrefs.data != null))
			{
				if(joyPrefs.data.usingJoystick != null)
					Ax.usingJoystick = joyPrefs.data.usingJoystick;
				else
					Ax.usingJoystick = false;
				joyPrefs.destroy();
			}
			
			if (Ax.usingJoystick)
				Ax.joystick = new AxJoystick();
			else 
				Ax.joystick = new AxJoystickEmpty();
		...
		static internal function reset():void
		{
			...
			Ax.timeScale = 1.0;
			Ax.elapsed = 0;
			Ax.globalSeed = Math.random();
			Ax.worldBounds = new AxRect(-10,-10,Ax.width+20,Ax.height+20);
			Ax.worldDivisions = 6;
			var debugPathDisplay:DebugPathDisplay = Ax.getPlugin(DebugPathDisplay) as DebugPathDisplay;
		...
		static internal function updateInput():void
		{
			...
			if(!_game._debuggerUp || !_game._debugger.hasMouse)
				Ax.mouse.update(Ax._game.mouseX,Ax._game.mouseY);
		}
		...
		{
			var cam:AxCamera;
			var cams:Array = Ax.cameras;
			var i:uint = 0;
			var l:uint = cams.length;
			while(i < l)
			{
				cam = cams[i++] as AxCamera;
		...
		static internal function updatePlugins():void
		{
			...
			while(i < l)
			{
				plugin = pluginList[i++] as AxBasic;
