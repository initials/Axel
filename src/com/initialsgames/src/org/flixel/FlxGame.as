package org.flixel
{
	...
	import org.flixel.system.AxReplay;

	/**
	 * AxGame is the heart of all flixel games, and contains a bunch of basic game loops and things.
	 * It is a long and sloppy file that you shouldn't have to worry about too much!
	 * It is basically only used to create your game object in the first place,
	 * after that AxG and AxState have all the useful stuff you actually need.
	 * 
	 * @author	Adam Atomic
	 */
	public class AxGame extends Sprite
	{
		 ...
		 * Also useful if you don't use AxPreloader!
		 * @default false
		 */
		 ...
		 */
		internal var _requestedState:AxState;
		/**
		 ...
		 */
		internal var _debugger:AxDebugger;
		/**
		 ...
		 */
		public function AxGame(GameSizeX:uint,GameSizeY:uint,InitialState:Class,Zoom:Number=1,GameFramerate:uint=60,FlashFramerate:uint=30,UseSystemCursor:Boolean=false)
		{
			...
			Ax.init(this,GameSizeX,GameSizeY,Zoom);
			Ax.framerate = GameFramerate;
			Ax.flashFramerate = FlashFramerate;
			_accumulator = _step;
			_total = 0;
			_state = null;
		...
		internal function showSoundTray(Silent:Boolean=false):void
		{
			...
			var globalVolume:uint = Math.round(Ax.volume*10);
			if(Ax.mute)
				globalVolume = 0;
		...
		protected function onKeyUp(FlashEvent:KeyboardEvent):void
		{
			...
			if(!Ax.mobile)
			{
				...
				if(useSoundHotKeys)
				{
					...
					switch(c)
					{
						...
						case 96:
							Ax.mute = !Ax.mute;
							if(Ax.volumeHandler != null)
								Ax.volumeHandler(Ax.mute?0:Ax.volume);
							showSoundTray();
							return;
						case 109:
						case 189:
							Ax.mute = false;
				    		Ax.volume = Ax.volume - 0.1;
				    		showSoundTray();
							return;
						case 107:
						case 187:
			...
			if(_replaying)
				return;
			Ax.keys.handleKeyUp(FlashEvent);
		}
		...
		protected function onKeyDown(FlashEvent:KeyboardEvent):void
		{
			...
			if(_replaying && (_replayCancelKeys != null) && (FlashEvent.keyCode != 192) && (FlashEvent.keyCode != 220)) //&& (_debugger == null) 
			{
				...
				while(i < l)
				{
					...
					if((replayCancelKey == "ANY") || (Ax.keys.getKeyCode(replayCancelKey) == FlashEvent.keyCode))
					{
			...
			Ax.keys.handleKeyDown(FlashEvent);
		}
		...
		protected function onMouseDown(FlashEvent:MouseEvent):void
		{
			...
			Ax.mouse.handleMouseDown(FlashEvent);
		}
		...
		protected function onMouseUp(FlashEvent:MouseEvent):void
		{
			if((_debuggerUp && _debugger.hasMouse) || _replaying)
				return;
			Ax.mouse.handleMouseUp(FlashEvent);
		}
		...
		protected function onMouseWheel(FlashEvent:MouseEvent):void
		{
			if((_debuggerUp && _debugger.hasMouse) || _replaying)
				return;
			Ax.mouse.handleMouseWheel(FlashEvent);
		}
		...
		{			
			...
			if(!_lostFocus)
			{
				...
				AxBasic._VISIBLECOUNT = 0;
				draw();
				
				if(_debuggerUp)
				{
					_debugger.perf.flash(elapsedMS);
					_debugger.perf.visibleObjects(AxBasic._VISIBLECOUNT);
		...
		{ 
			...
			var timerManager:TimerManager = AxTimer.manager;
		...
		protected function step():void
		{
			...
			{
				_recordingRequested = false;
				_replay.create(Ax.globalSeed);
				_recording = true;
				if(_debugger != null)
				{
					_debugger.vcr.recording();
					Ax.log("FLIXEL: starting new flixel gameplay record.");
				}
			}
			...
			Ax.mouse.wheel = 0;
			if(_debuggerUp)
				_debugger.perf.activeObjects(AxBasic._ACTIVECOUNT);
		}
		...
		protected function updateSoundTray(MS:Number):void
		{
			...
			if(_soundTray != null)
			{
				...
				{
					_soundTray.y -= (MS/1000)*Ax.height*2;
					if(_soundTray.y <= -_soundTray.height)
					{
						_soundTray.visible = false;
						
						//Save sound preferences
						var soundPrefs:AxSave = new AxSave();
						if(soundPrefs.bind("flixel"))
						{
							if(soundPrefs.data.sound == null)
								soundPrefs.data.sound = new Object;
							soundPrefs.data.sound.mute = Ax.mute;
							soundPrefs.data.sound.volume = Ax.volume;
							soundPrefs.close();
						}
					}
				}
		...
		{			
			...
			Ax.elapsed = Ax.timeScale*(_step/1000);
			Ax.updateSounds();
			Ax.updatePlugins();
			_state.update();
			Ax.updateCameras();
		...
		protected function create(FlashEvent:Event):void
		{
			...
			{
				//Debugger overlay
				if(Ax.debug || forceDebugger)
				{
					_debugger = new AxDebugger(Ax.width*AxCamera.defaultZoom,Ax.height*AxCamera.defaultZoom);
					addChild(_debugger);
				}
		...
		protected function createSoundTray():void
		{
			...
			_soundTray.x = (Ax.width/2)*AxCamera.defaultZoom-(tmp.width/2)*_soundTray.scaleX;
			...
			if(soundPrefs.bind("flixel") && (soundPrefs.data.sound != null))
			{
				if(soundPrefs.data.sound.volume != null)
					Ax.volume = soundPrefs.data.sound.volume;
				if(soundPrefs.data.sound.mute != null)
					Ax.mute = soundPrefs.data.sound.mute;
				soundPrefs.destroy();
			}
		}
		...
		protected function createFocusScreen():void
		{
			...
			var screenWidth:uint = Ax.width*AxCamera.defaultZoom;
			var screenHeight:uint = Ax.height*AxCamera.defaultZoom;
			...
			var helper:uint = AxU.min(halfWidth,halfHeight)/3;
