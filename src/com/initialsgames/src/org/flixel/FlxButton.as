package org.flixel
{
	...
	public class AxButton extends AxSprite
	{
		 ...
		 */
		public function AxButton(X:Number=0,Y:Number=0,Label:String=null,OnClick:Function=null)
		{
			super(X,Y);
			if(Label != null)
			{
				label = new AxText(0,0,91,Label);
				label.setFormat(null,8,0x333333,"center");
				labelOffset = new AxPoint( -1, 3);
				regularText = Label;
		...
		protected function updateButton():void
		{
			...
			if(Ax.mouse.visible)
			{
				...
				while(i < l)
				{
					camera = cameras[i++] as AxCamera;
					Ax.mouse.getWorldPosition(camera,_point);
		...
		{
			if(SoundOver != null)
				soundOver = Ax.loadSound(SoundOver, SoundOverVolume);
			if(SoundOut != null)
				soundOut = Ax.loadSound(SoundOut, SoundOutVolume);
			if(SoundDown != null)
				soundDown = Ax.loadSound(SoundDown, SoundDownVolume);
			if(SoundUp != null)
				soundUp = Ax.loadSound(SoundUp, SoundUpVolume);
		}
