package org.flixel
{
	...
	public class AxU
	{
		 ...
		 * 
		 * @return	A <code>uint</code> to be passed to <code>AxU.endProfile()</code>.
		 */
		 ...
		 * if you call say <code>var results:String = AxU.formatMoney(10,false);</code>
		...
		{
			if(Acceleration != 0)
				Velocity += Acceleration*Ax.elapsed;
			else if(Drag != 0)
			{
				var drag:Number = Drag*Ax.elapsed;
				if(Velocity - drag > 0)
		...
		//*** NOTE: THESE LAST THREE FUNCTIONS REQUIRE AxPOINT ***//
		 ...
		 * @param	Point	Optional <code>AxPoint</code> to store the results in.
		 * 
		 * @return	A <code>AxPoint</code> containing the coordinates of the rotated point.
		 */
		static public function rotatePoint(X:Number, Y:Number, PivotX:Number, PivotY:Number, Angle:Number,Point:AxPoint=null):AxPoint
		{
			var sin:Number = 0;
			var cos:Number = 0;
		 ...
		 */
		static public function getAngle(Point1:AxPoint, Point2:AxPoint):Number
		{
