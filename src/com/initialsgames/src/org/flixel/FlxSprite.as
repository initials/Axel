package org.flixel
{
	...
	import org.flixel.system.AxAnim;
	
	/**
	 * The main "game object" class, the sprite is a <code>AxEntity</code>
	 * with a bunch of graphics options and abilities, like animation and stamping.
	 * 
	 * @author	Adam Atomic
	 */
	public class AxSprite extends AxEntity
	{
		...
		*/
		 ...
		 */ 
		public var originalPosition:AxPoint;
		
		/**
		 * Creates a white 8x8 square <code>AxSprite</code> at the specified position.
		 ...
		 */
		public function AxSprite(X:Number=0,Y:Number=0,SimpleGraphic:Class=null)
		{
			super(X,Y);
			
			originalPosition = new AxPoint(X, Y);
		 ...
		 * @param	Width		Optional, specify the width of your sprite (helps AxSprite figure out what to do with non-square sprites or sprite sheets).
		 * @param	Height		Optional, specify the height of your sprite (helps AxSprite figure out what to do with non-square sprites or sprite sheets).
		 * @param	Unique		Optional, whether the graphic should be a unique instance in the graphics cache.  Default is false.
		 * 
		 * @return	This AxSprite instance (nice for chaining stuff together, if you're into that).
		 */
		public function loadGraphic(Graphic:Class,Animated:Boolean=false,Reverse:Boolean=false,Width:uint=0,Height:uint=0,Unique:Boolean=false):AxSprite
		{
			_bakedRotation = 0;
			_pixels = Ax.addBitmap(Graphic,Reverse,Unique);
			if(Reverse)
		 ...
		 * @return	This AxSprite instance (nice for chaining stuff together, if you're into that).
		 */
		public function loadRotatedGraphic(Graphic:Class, Rotations:uint=16, Frame:int=-1, AntiAliasing:Boolean=false, AutoBuffer:Boolean=false):AxSprite
		{
			//Create the brush and canvas
			var rows:uint = Math.sqrt(Rotations);
			var brush:BitmapData = Ax.addBitmap(Graphic);
			if(Frame >= 0)
			{
			...
			if(AutoBuffer)
				max *= 1.5;
			var columns:uint = AxU.ceil(Rotations/rows);
			width = max*columns;
			height = max*rows;
			var key:String = String(Graphic) + ":" + Frame + ":" + width + "x" + height;
			var skipGen:Boolean = Ax.checkBitmapCache(key);
			_pixels = Ax.createBitmap(width, height, 0, true, key);
		 ...
		 * @return	This AxSprite instance (nice for chaining stuff together, if you're into that).
		 */
		public function makeGraphic(Width:uint,Height:uint,Color:uint=0xffffffff,Unique:Boolean=false,Key:String=null):AxSprite
		{
			_bakedRotation = 0;
			_pixels = Ax.createBitmap(Width,Height,Color,Unique,Key);
			width = frameWidth = _pixels.width;
			height = frameHeight = _pixels.height;
			resetHelpers();
			return this;
		...
		override public function draw():void
		{
			...
			if (_fading) {
				_fadeCounter += Ax.elapsed/_fadeDuration;
		...
		/**
		 * This function draws or stamps one <code>AxSprite</code> onto another.
		 ...
		 */
		public function stamp(Brush:AxSprite,X:int=0,Y:int=0):void
		{
			Brush.drawFrame();
		...
		public function drawLine(StartX:Number,StartY:Number,EndX:Number,EndY:Number,Color:uint,Thickness:uint=1):void
		{
			...
			_pixels.draw(Ax.flashGfxSprite);
			dirty = true;
		}
		 ...
		 * This function is called automatically by <code>AxSprite.postUpdate()</code>.
		 */
		...
		{
			_animations.push(new AxAnim(Name,Frames,FrameRate,Looped));
		}
		...
		public function play(AnimName:String,Force:Boolean=false):void
		{
			...
			Ax.log("WARNING: No animation called \""+AnimName+"\"");
		}
		...
		{
			_curAnim = null;
			_curIndex = int(Ax.random()*(_pixels.width/frameWidth));
			dirty = true;
		}
			...
			while(row < rows)
			{
				...
				while(column < columns)
				{
					if(_pixels.getPixel32(column,row) == Color)
					{
						...
						if(FetchPositions)
							positions.push(new AxPoint(column,row));
						dirty = true;
					}
					column++;
				}
		 ...
		 * @param	FetchPositions		If true, you will return an array of positions <code>AxPoint</code>. False will return a CSV array of 0s and 1s.
		 */
		...
		/**
		 * Set <code>facing</code> using <code>AxSprite.LEFT</code>,<code>RIGHT</code>,
...
/*		public function fade(Color:uint, Duration:Number):void
		 ...
		 * Differs from <code>AxEntity</code>'s implementation
		 ...
		 */
		override public function onScreen(Camera:AxCamera=null):Boolean
		{
			if(Camera == null)
				Camera = Ax.camera;
		...
		/**
		 * Checks to see if a point in 2D world space overlaps this <code>AxSprite</code> object's current displayed pixels.
		 ...
		 */
		public function pixelsOverlapPoint(Point:AxPoint,Mask:uint=0xFF,Camera:AxCamera=null):Boolean
		{
			if(Camera == null)
				Camera = Ax.camera;
			getScreenXY(_point,Camera);
