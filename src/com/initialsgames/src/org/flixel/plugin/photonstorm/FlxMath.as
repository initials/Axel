 ...
 * 
 * v1.7 Added mouseInAxRect
 * v1.6 Added wrapAngle, angleLimit and more documentation
 * v1.5 Added pointInCoordinates, pointInAxRect and pointInRectangle
...
package org.flixel.plugin.photonstorm 
{
	...
	public class AxMath
	{
		 ...
		 * @param	pointX		The X value to test
		 * @param	pointY		The Y value to test
		 * @param	rect		The AxRect to test within
		 * @return	true if pointX/pointY is within the AxRect, otherwise false
		 */
		public static function pointInAxRect(pointX:int, pointY:int, rect:AxRect):Boolean
		{
		 ...
		 * @param	rect			The AxRect to test within. If this is null for any reason this function always returns true.
		 * 
		 * @return	true if mouse is within the AxRect, otherwise false
		 */
		public static function mouseInAxRect(useWorldCoords:Boolean, rect:AxRect):Boolean
		{
			if (rect == null)
			{
				return true;
			...
			if (useWorldCoords)
			{
				return pointInAxRect(Ax.mouse.x, Ax.mouse.y, rect);
			}
			else
			{
				return pointInAxRect(Ax.mouse.screenX, Ax.mouse.screenY, rect);
			}
		}
		 ...
		 * Should be called whenever the angle is updated on the AxSprite to stop it from going insane.
		 * 
