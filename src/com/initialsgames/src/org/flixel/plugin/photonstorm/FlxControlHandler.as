...
package org.flixel.plugin.photonstorm 
{
	...
	/**
	 * Makes controlling an AxSprite with the keyboard a LOT easier and quicker to set-up!<br>
	 ...
	 */
	public class AxControlHandler
	{
		//	Used by the AxControl plugin
		public var enabled:Boolean = false;
		
		private var entity:AxSprite = null;
		...
		private var jumpSurface:uint; 		// The surfaces from AxEntity they can jump from (i.e. AxEntity.FLOOR)
		...
		//	Sounds
		private var jumpSound:AxSound = null;
		private var fireSound:AxSound = null;
		private var walkSound:AxSound = null;
		private var thrustSound:AxSound = null;
		...
		/**
		 * Sets the AxSprite to be controlled by this class, and defines the initial movement and stopping types.<br>
		 ...
		 */
		public function AxControlHandler(source:AxSprite, movementType:int, stoppingType:int, updateFacing:Boolean = false, enableArrowKeys:Boolean = true)
		{
			entity = source;
		...
		/**
		 * Sets the maximum speed (in pixels per second) that the AxSprite can rotate.<br>
		 * When the AxSprite is accelerating (movement type MOVEMENT_ACCELERATES) its speed won't increase above this value.<br>
		 * However Flixel allows the velocity of an AxSprite to be set to anything. So if you'd like to check the value and restrain it, then enable "limitVelocity".
		 * 
		 * @param	speed			The maximum speed in pixels per second in which the sprite can rotate
		 * @param	limitVelocity	If true the angular velocity of the AxSprite will be checked and kept within the limit. If false it can be set to anything.
		 */
		...
		public function setRotationLimits(minimumAngle:int, maximumAngle:int):void
		{
			...
			{
				throw new Error("AxControlHandler setRotationLimits: Invalid Minimum / Maximum angle");
			}
			else
			{
		...
		/**
		 * Sets the maximum speed (in pixels per second) that the AxSprite can move. You can set the horizontal and vertical speeds independantly.<br>
		 * When the AxSprite is accelerating (movement type MOVEMENT_ACCELERATES) its speed won't increase above this value.<br>
		 * However Flixel allows the velocity of an AxSprite to be set to anything. So if you'd like to check the value and restrain it, then enable "limitVelocity".
		 * 
		 ...
		 * @param	limitVelocity	If true the velocity of the AxSprite will be checked and kept within the limit. If false it can be set to anything.
		 */
		 ...
		 * 
		 * @param	jump	The AxSound to play when the user jumps
		 * @param	fire	The AxSound to play when the user fires
		 * @param	walk	The AxSound to play when the user walks
		 * @param	thrust	The AxSound to play when the user thrusts
		 */
		public function setSounds(jump:AxSound = null, fire:AxSound = null, walk:AxSound = null, thrust:AxSound = null):void
		{
			if (jump)
			{
				jumpSound = jump;
		 ...
		 * @param	keymode			The AxControlHandler KEYMODE value (KEYMODE_PRESSED, KEYMODE_JUST_DOWN, KEYMODE_RELEASED)
		 ...
		 * @param	surface			A bitwise combination of all valid surfaces the Sprite can jump off (from AxEntity, such as AxEntity.FLOOR)
		...
		private function moveUp():Boolean
		{
			...
			if (Ax.keys.pressed(upKey))
			{
				move = true;
				
				if (yFacing)
				{
					entity.facing = AxEntity.UP;
				}
		...
		private function moveDown():Boolean
		{
			...
			if (Ax.keys.pressed(downKey))
			{
				move = true;
				
				if (yFacing)
				{
					entity.facing = AxEntity.DOWN;
				}
		...
		private function moveLeft():Boolean
		{
			...
			if (Ax.keys.pressed(leftKey))
			{
				move = true;
				
				if (xFacing)
				{
					entity.facing = AxEntity.LEFT;
				}
		...
		private function moveRight():Boolean
		{
			...
			if (Ax.keys.pressed(rightKey))
			{
				move = true;
				
				if (xFacing)
				{
					entity.facing = AxEntity.RIGHT;
				}
		...
		private function moveAntiClockwise():Boolean
		{
			...
			if (Ax.keys.pressed(antiClockwiseKey))
			{
				move = true;
				...
				{
					//entity.angle = AxMath.angleLimit(entity.angle, minAngle, maxAngle);
				}
			}
		...
		private function moveClockwise():Boolean
		{
			...
			if (Ax.keys.pressed(clockwiseKey))
			{
				move = true;
		...
		private function moveThrust():Boolean
		{
			...
			if (Ax.keys.pressed(thrustKey))
			{
				move = true;
				
				var motion:AxPoint = AxVelocity.velocityFromAngle(entity.angle, thrustSpeed);
		...
		private function moveReverse():Boolean
		{
			...
			if (Ax.keys.pressed(reverseKey))
			{
				move = true;
				
				var motion:AxPoint = AxVelocity.velocityFromAngle(entity.angle, reverseSpeed);
		...
		private function runFire():Boolean
		{
			...
			//	1 = Just Down
			//	2 = Just Released
			if ((fireKeyMode == 0 && Ax.keys.pressed(fireKey)) || (fireKeyMode == 1 && Ax.keys.justPressed(fireKey)) || (fireKeyMode == 2 && Ax.keys.justReleased(fireKey)))
			{
				if (fireRate > 0)
				{
		...
		private function runJump():Boolean
		{
			...
			if ((jumpKeyMode == KEYMODE_PRESSED && Ax.keys.pressed(jumpKey)) || (jumpKeyMode == KEYMODE_JUST_DOWN && Ax.keys.justPressed(jumpKey)) || (jumpKeyMode == KEYMODE_RELEASED && Ax.keys.justReleased(jumpKey)))
			{
		...
		/**
		 * Called by the AxControl plugin
		 */
