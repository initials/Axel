 ...
 * v1.2 Now works fully with AxMouseControl to be completely clickable and draggable!
...
package org.flixel.plugin.photonstorm 
{
	...
	/**
	 * An enhanced AxSprite that is capable of receiving mouse clicks, being dragged and thrown, mouse springs, gravity and other useful things
	 */
	public class AxExtendedSprite extends AxSprite
	{
		/**
		 * Used by AxMouseControl when multiple sprites overlap and register clicks, and you need to determine which sprite has priority
		 */
		...
		/**
		 * Function called when the mouse is pressed down on this sprite. Function is passed these parameters: obj:AxExtendedSprite, x:int, y:int
		 * @default null
		 */
		public var mousePressedCallback:Function;
		
		/**
		 * Function called when the mouse is released from this sprite. Function is passed these parameters: obj:AxExtendedSprite, x:int, y:int
		 * @default null
		 */
		...
		/**
		 * Function called when the mouse starts to drag this sprite. Function is passed these parameters: obj:AxExtendedSprite, x:int, y:int
		 * @default null
		 */
		public var mouseStartDragCallback:Function;
		
		/**
		 * Function called when the mouse stops dragging this sprite. Function is passed these parameters: obj:AxExtendedSprite, x:int, y:int
		 * @default null
		 */
		public var mouseStopDragCallback:Function;
		
		/**
		 * An AxRect region of the game world within which the sprite is restricted during mouse drag
		 * @default null
		 */
		public var boundsRect:AxRect = null;
		
		/**
		 * An AxSprite the bounds of which this sprite is restricted during mouse drag
		 * @default null
		 */
		public var boundsSprite:AxSprite = null;
		...
		/**
		 * Creates a white 8x8 square <code>AxExtendedSprite</code> at the specified position.
		 ...
		 */
		public function AxExtendedSprite(X:Number = 0, Y:Number = 0, SimpleGraphic:Class = null)
		{
		...
		{
			if (Ax.getPlugin(AxMouseControl) == null)
			{
				throw Error("AxExtendedSprite.enableMouseClicks called but AxMouseControl plugin not activated");
			}
		 ...
		 * @param	pixelPerfect		If true it will use a pixel perfect test to see if you clicked the Sprite. False uses the bounding box.
		 * @param	alphaThreshold		If using pixel perfect collision this specifies the alpha level from 0 to 255 above which a collision is processed (default 255)
		 * @param	boundsRect			If you want to restrict the drag of this sprite to a specific AxRect, pass the AxRect here, otherwise it's free to drag anywhere
		 * @param	boundsSprite		If you want to restrict the drag of this sprite to within the bounding box of another sprite, pass it here
		 */
		public function enableMouseDrag(lockCenter:Boolean = false, pixelPerfect:Boolean = false, alphaThreshold:uint = 255, boundsRect:AxRect = null, boundsSprite:AxSprite = null):void
		{
			if (Ax.getPlugin(AxMouseControl) == null)
			{
				throw Error("AxExtendedSprite.enableMouseDrag called but AxMouseControl plugin not activated");
			}
		...
		public function disableMouseDrag():void
		{
			if (isDragged)
			{
				AxMouseControl.dragTarget = null;
				AxMouseControl.isDragging = false;
			}
		...
		* Restricts this sprite to drag movement only on the given axis. Note: If both are set to false the sprite will never move!
		 ...
		 * 
		 * @param	xFactor		The sprites velocity is set to AxMouseControl.speedX * xFactor. Try a value around 50+
		 * @param	yFactor		The sprites velocity is set to AxMouseControl.speedY * yFactor. Try a value around 50+
		 */
		public function enableMouseThrow(xFactor:int, yFactor:int):void
		{
			if (Ax.getPlugin(AxMouseControl) == null)
			{
				throw Error("AxExtendedSprite.enableMouseThrow called but AxMouseControl plugin not activated");
			}
		 ...
		 * 
		 * @return	The MouseSpring object if you wish to perform further chaining on it. Also available via AxExtendedSprite.mouseSpring
		 */ 
		public function enableMouseSpring(onPressed:Boolean = true, retainVelocity:Boolean = false, tension:Number = 0.1, friction:Number = 0.95, gravity:Number = 0):MouseSpring
		{
			if (Ax.getPlugin(AxMouseControl) == null)
			{
				throw Error("AxExtendedSprite.enableMouseSpring called but AxMouseControl plugin not activated");
			}
		...
		override public function update():void
		{
			...
			if (isPressed == false && Ax.mouse.justPressed())
			{
				checkForClick();
			}
		...
		{
			//Ax.mouse.getWorldPosition(null, tempPoint);
			
			if (allowHorizontalDrag)
			{
				x = int(Ax.mouse.x) - dragOffsetX;
			}
			
			if (allowVerticalDrag)
			{
				y = int(Ax.mouse.y) - dragOffsetY;
			}
		...
		/**
		 * Checks if the mouse is over this sprite and pressed, then does a pixel perfect check if needed and adds it to the AxMouseControl check stack
		 */
		private function checkForClick():void
		{
			if (mouseOver && Ax.mouse.justPressed())
			{
				...
				{
					AxMouseControl.addToStack(this);
					return;
				}
				
				if (clickPixelPerfect && AxCollision.pixelPerfectPointCheck(Ax.mouse.x, Ax.mouse.y, this, clickPixelPerfectAlpha))
				{
					AxMouseControl.addToStack(this);
					return;
				}
				
				if (dragPixelPerfect && AxCollision.pixelPerfectPointCheck(Ax.mouse.x, Ax.mouse.y, this, dragPixelPerfectAlpha))
				{
					AxMouseControl.addToStack(this);
					return;
				}
		...
		/**
		 * Called by AxMouseControl when this sprite is clicked. Should not usually be called directly.
		 */
		...
		/**
		 * Called by AxMouseControl when this sprite is released from a click. Should not usually be called directly.
		 */
		public function mouseReleasedHandler():void
		{
			...
			if (throwable)
			{
				velocity.x = AxMouseControl.speedX * throwXFactor;
				velocity.y = AxMouseControl.speedY * throwYFactor;
			}
		...
		/**
		 * Called by AxMouseControl when Mouse Drag starts on this Sprite. Should not usually be called directly.
		 */
		public function startDrag():void
		{
			...
			{
				dragOffsetX = int(Ax.mouse.x) - x;
				dragOffsetY = int(Ax.mouse.y) - y;
			}
			else
			{
		...
		/**
		 * Called by AxMouseControl when Mouse Drag is stopped on this Sprite. Should not usually be called directly.
		 */
		...
		/**
		 * Returns an AxPoint consisting of this sprites world x/y coordinates
		 */
		public function get point():AxPoint
		{
			return _point;
		}
		
		public function set point(p:AxPoint):void
		{
			_point = p;
		}
		...
		{
			return AxMath.pointInCoordinates(Ax.mouse.x, Ax.mouse.y, x, y, width, height);
		}
		...
		/**
		 * Returns an AxRect consisting of the bounds of this Sprite.
		 */
		public function get rect():AxRect
		{
			_rect.x = x;
			_rect.y = y;
			_rect.width = width;
