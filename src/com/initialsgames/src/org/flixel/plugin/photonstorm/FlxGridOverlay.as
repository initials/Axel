...
package org.flixel.plugin.photonstorm 
{
	...
	public class AxGridOverlay 
	{
		public function AxGridOverlay() 
		{
		}
		
		/**
		 * Creates an AxSprite of the given width and height filled with a checkerboard pattern.<br />
		 ...
		 * @param	width			The width of the AxSprite. If -1 it will be the size of the game (Ax.width)
		 * @param	height			The height of the AxSprite. If -1 it will be the size of the game (Ax.height)
		 ...
		 * 
		 * @return	AxSprite of given width/height
		 */
		public static function create(cellWidth:int, cellHeight:int, width:int = -1, height:int = -1, addLegend:Boolean = false, alternate:Boolean = true, color1:uint = 0xffe7e6e6, color2:uint = 0xffd9d5d5):AxSprite
		{
			if (width == -1)
			{
				width = Ax.width;
			...
			var output:AxSprite = new AxSprite().makeGraphic(width, height);
		...
		/**
		 * Creates a checkerboard pattern of the given width/height and overlays it onto the given AxSprite.<br />
		 * Each grid cell is the specified width and height, and alternates between two colors.<br />
		 * If alternate is true each row of the pattern will be offset, for a proper checkerboard style. If false each row will be the same colour, creating a striped-pattern effect.<br />
		 * So to create an 8x8 grid you'd call create(8,8,
		 * 
		 * @param	source			The AxSprite you wish to draw the grid on-top of. This updates its pixels value, not just the current frame (don't use animated sprites!)
		 * @param	cellWidth		The grid cell width
		 * @param	cellHeight		The grid cell height
		 * @param	width			The width of the AxSprite. If -1 it will be the size of the game (Ax.width)
		 * @param	height			The height of the AxSprite. If -1 it will be the size of the game (Ax.height)
		 ...
		 * @param	color2			The second fill colour in 0xAARRGGBB format
		 * 
		 * @return	The modified source AxSprite
		 */
		public static function overlay(source:AxSprite, cellWidth:int, cellHeight:int, width:int = -1, height:int = -1, addLegend:Boolean = false, alternate:Boolean = true, color1:uint = 0x88e7e6e6, color2:uint = 0x88d9d5d5):AxSprite
		{
			if (width == -1)
			{
				width = Ax.width;
		...
		public static function addLegend(source:AxSprite, cellWidth:int, cellHeight:int, xAxis:Boolean = true, yAxis:Boolean = true):AxSprite
		{
			if (cellWidth > source.width)
			{
				throw Error("cellWidth larger than AxSprites width");
				return source;
			}
			
			if (cellHeight > source.height)
			{
				throw Error("cellHeight larger than AxSprites height");
				return source;
			}
			
			if (source.width < cellWidth || source.height < cellHeight)
			{
				throw Error("source AxSprite width or height smaller than requested cell width or height");
				return source;
			}
