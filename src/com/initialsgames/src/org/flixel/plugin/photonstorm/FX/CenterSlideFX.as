...
package org.flixel.plugin.photonstorm.FX 
{
	...
	public class CenterSlideFX extends BaseFX
	{
		...
		/**
		 * Creates a new CenterSlide effect from the given AxSprite. The original sprite remains unmodified.<br>
		 * The resulting AxSprite will take on the same width / height and x/y coordinates of the source AxSprite.
		 * 
		 * @param	source				The AxSprite providing the image data for this effect. The resulting AxSprite takes on the source width, height and x/y position.
		 ...
		 * @param	backgroundColor		The background colour of the AxSprite the effect is drawn in to (default 0x0 = transparent)
		 * 
		 * @return	An AxSprite with the effect running through it, which should be started with a call to CenterSlideFX.start()
		 */
		public function createFromAxSprite(source:AxSprite, direction:uint = 0, pixels:uint = 1, backgroundColor:uint = 0x0):AxSprite
		{
		 ...
		 * @return	An AxSprite with the effect running through it, which should be started with a call to CenterSlideFX.start()
		 */
		public function createFromClass(source:Class, x:int, y:int, direction:uint = 0, pixels:uint = 1, backgroundColor:uint = 0x0):AxSprite
		{
		 ...
		 * @return	An AxSprite with the effect running through it, which should be started with a call to CenterSlideFX.start()
		 */
		public function createFromBitmapData(source:BitmapData, x:int, y:int, direction:uint = 0, pixels:uint = 1, backgroundColor:uint = 0x0):AxSprite
		{
			return create(source, x, y, direction, pixels, backgroundColor);
		}
		
		private function create(source:BitmapData, x:int, y:int, direction:uint = 0, pixels:uint = 1, backgroundColor:uint = 0x0):AxSprite
		{
			sprite = new AxSprite(x, y).makeGraphic(source.width, source.height, backgroundColor);
