...
package org.flixel.plugin.photonstorm 
{
	 ...
	 */
	public class AxGradient
	{
		
		public function AxGradient() 
		{
		}
		...
		public static function createGradientMatrix(width:int, height:int, colors:Array, chunkSize:int = 1, rotation:int = 90):Object
		{
			...
			var rot:Number = AxMath.asRadians(rotation);
			...
			{
				alpha.push(AxColor.getAlphaFloat(colors[ai]));
			}
		...
		/**
		 * Creates an AxSprite of the given width/height with a colour gradient flowing through it.
		 * 
		 * @param	width		The width of the AxSprite (and therefore gradient)
		 * @param	height		The height of the AxSprite (and therefore gradient)
		 ...
		 * 
		 * @return	An AxSprite containing your gradient (if valid parameters given!)
		 */
		public static function createGradientAxSprite(width:int, height:int, colors:Array, chunkSize:int = 1, rotation:int = 90, interpolate:Boolean = true):AxSprite
		{
			var data:BitmapData = createGradientBitmapData(width, height, colors, chunkSize, rotation, interpolate);
			
			var dest:AxSprite = new AxSprite().makeGraphic(width, height);
		...
		/**
		 * Creates a new gradient and overlays that on-top of the given AxSprite at the destX/destY coordinates (default 0,0)<br />
		 * Use low alpha values in the colours to have the gradient overlay and not destroy the image below
		 * 
		 * @param	dest		The AxSprite to overlay the gradient onto
		 * @param	width		The width of the AxSprite (and therefore gradient)
		 * @param	height		The height of the AxSprite (and therefore gradient)
		 * @param	colors		An array of colour values for the gradient to cycle through
		 ...
		 * @param	rotation	Angle of the gradient in degrees. 90 = top to bottom, 180 = left to right. Any angle is valid
		 * @param	interpolate	Interpolate the colours? True uses RGB interpolation, false uses linear RGB
		 * @return	The composited AxSprite (for chaining, if you need)
		 */
		public static function overlayGradientOnAxSprite(dest:AxSprite, width:int, height:int, colors:Array, destX:int = 0, destY:int = 0, chunkSize:int = 1, rotation:int = 90, interpolate:Boolean = true):AxSprite
		{
			if (width > dest.width)
			{
				width = dest.width;
			...
			var source:AxSprite = createGradientAxSprite(width, height, colors, chunkSize, rotation, interpolate);
