...
package org.flixel.plugin.photonstorm.FX 
{
	...
	/**
	 * Creates a sine-wave effect through an AxSprite which can be applied vertically or horizontally
	 */
	public class SineWaveFX extends BaseFX
	{
		...
		/**
		 * Creates a new SineWaveFX Effect from the given AxSprite. The original sprite remains unmodified.<br>
		 * The resulting AxSprite will take on the same width / height and x/y coordinates of the source AxSprite.<br>
		 * For really cool effects you can SineWave an AxSprite that is constantly updating (either through animation or an FX chain).
		 * 
		 * @param	source				The AxSprite providing the image data for this effect. The resulting AxSprite takes on the source width, height, x/y positions and scrollfactor.
		 ...
		 * @return	An AxSprite with the effect running through it, which should be started with a call to SineWaveFX.start()
		 */
		public function createFromAxSprite(source:AxSprite, type:uint, size:uint, length:uint, frequency:uint = 2, pixelsPerChunk:uint = 1, updateFrame:Boolean = false, backgroundColor:uint = 0x0):AxSprite
		{
			var result:AxSprite = create(source.pixels, source.x, source.y, type, size, length, frequency, pixelsPerChunk, backgroundColor);
		 ...
		 * If you need to update the source data at run-time then use createFromAxSprite
		 * 
		 * @param	source				The Class providing the bitmapData for this effect, usually from an Embedded bitmap.
		 * @param	x					The x coordinate (in game world pixels) that the resulting AxSprite will be created at.
		 * @param	y					The x coordinate (in game world pixels) that the resulting AxSprite will be created at.
		 * @param	type				WAVETYPE_VERTICAL_SINE, WAVETYPE_VERTICAL_COSINE, WAVETYPE_HORIZONTAL_SINE or WAVETYPE_HORIZONTAL_COSINE
		 * @param	size				The size in pixels of the sine wave. Either the height of the wave or the width (for vertical or horizontal waves)
		 * @param	length				The length of the wave in pixels. You should usually set this to the width or height of the source image, or a multiple of it.
		 * @param	frequency			The frequency of the peaks in the wave. MUST BE AN EVEN NUMBER! 2, 4, 6, 8, etc.
		 ...
		 * @return	An AxSprite with the effect running through it, which should be started with a call to SineWaveFX.start()
		 */
		public function createFromClass(source:Class, x:int, y:int, type:uint, size:uint, length:uint, frequency:uint = 2, pixelsPerChunk:uint = 1, backgroundColor:uint = 0x0):AxSprite
		{
			var result:AxSprite = create((new source).bitmapData, x, y, type, size, length, frequency, pixelsPerChunk, backgroundColor);
		 ...
		 * @return	An AxSprite with the effect running through it, which should be started with a call to SineWaveFX.start()
		 */
		public function createFromBitmapData(source:BitmapData, x:int, y:int, type:uint, size:uint, length:uint, frequency:uint = 2, pixelsPerChunk:uint = 1, backgroundColor:uint = 0x0):AxSprite
		{
			var result:AxSprite = create(source, x, y, type, size, length, frequency, pixelsPerChunk, backgroundColor);
		...
		/**
		 * Internal function fed from createFromAxSprite / createFromClass / createFromBitmapData
		 * 
		 * @param	source				The bitmapData image to use for this effect.
		 * @param	x					The x coordinate (in game world pixels) that the resulting AxSprite will be created at.
		 * @param	y					The x coordinate (in game world pixels) that the resulting AxSprite will be created at.
		 ...
		 * @return	An AxSprite with the effect running through it, which should be started with a call to SineWaveFX.start()
		 */
		private function create(source:BitmapData, x:int, y:int, type:uint, size:uint, length:uint, frequency:uint = 2, pixelsPerChunk:uint = 1, backgroundColor:uint = 0x0):AxSprite
		{
			...
			//	The AxSprite into which the sine-wave effect is drawn
			
			if (waveVertical)
			{
				sprite = new AxSprite(x, y).makeGraphic(source.width, source.height + (waveSize * 3), backgroundColor);
			}
			else
			{
				sprite = new AxSprite(x, y).makeGraphic(source.width + (waveSize * 3), source.height, backgroundColor);
			}
		...
		public function updateWaveData(type:uint, size:uint, length:uint, frequency:uint = 2, pixelsPerChunk:uint = 1):void
		{
			...
			waveData = AxMath.sinCosGenerator(waveLength, waveSize, waveSize, waveFrequency);
			
			if (waveType == WAVETYPE_VERTICAL_COSINE || waveType == WAVETYPE_HORIZONTAL_COSINE)
			{
				waveData = AxMath.getCosTable();
			}
		}
		...
		/**
		 * Called by the AxSpecialFX plugin. Should not be called directly.
		 */
