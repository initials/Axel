...
package org.flixel.plugin.photonstorm.FX 
{
	...
	/**
	 * Creates a 2D or 3D Star Field effect on an AxSprite for use in your game.
	 */
	public class StarfieldFX extends BaseFX
	{
		 ...
		 */
		public function create(x:int, y:int, width:uint, height:uint, quantity:uint = 200, type:int = 1, updateInterval:int = 20):AxSprite
		{
			sprite = new AxSprite(x, y).makeGraphic(width, height, backgroundColor);
			...
			if (type == STARFIELD_TYPE_2D)
			{
				depthColours = AxGradient.createGradientArray(1, 5, [0xff585858, 0xffF4F4F4]);
			}
			else
			{
				depthColours = AxGradient.createGradientArray(1, 300, [0xff292929, 0xffffffff]);
			}
		...
		public function setStarDepthColors(depth:int, lowestColor:uint = 0xff585858, highestColor:uint = 0xffF4F4F4):void
		{
			...
			depthColours = AxGradient.createGradientArray(1, depth, [lowestColor, highestColor]);
		...
		private function update3DStarfield():void
		{
			for each (var star:Object in stars)
			{
				...
				//canvas.setPixel32(star.x, star.y, AxColor.getColor32(255, star.alpha, star.alpha, star.alpha));
