...
package org.flixel.plugin.photonstorm 
{
	 ...
	 */
	public class AxScreenGrab extends AxBasic
	{
		...
		/**
		 * Specify which key will capture a screen shot. Use the String value of the key in the same way Ax.keys does (so "F1" for example)<br />
		...
		public static function grab(captureRegion:Rectangle = null, saveToFile:Boolean = false, hideMouse:Boolean = false):Bitmap
		{
			...
			{
				bounds = new Rectangle(0, 0, Ax.stage.stageWidth, Ax.stage.stageHeight);
			}
			...
			theBitmap.bitmapData.draw(Ax.stage, m);
		...
		override public function update():void
		{
			if (hotkey != "")
			{
				if (Ax.keys.justReleased(hotkey))
				{
					trace("key pressed");
					grab();
				}
