...
package org.flixel.plugin.photonstorm
{
	...
	public class AxFlectrum extends AxGroup
	{
		...
		public var back:AxSprite;
		public var front:AxSprite;
		...
		public function init(x:int, y:int, soundEx:SoundEx, columns:int = 15, columnSize:int = 10, columnSpacing:int = 0, rows:int = 32, rowSize:int = 3, rowSpacing:int = 0):void
		{
			...
			front = new AxSprite().makeGraphic(1, 1, 0x0);
			front.solid = false;
			
			back = new AxSprite().makeGraphic(1, 1, 0x0);
			back.solid = false;
			back.visible = false;
		...
		{
			if (value == m_direction || !AxFlectrum[value.toUpperCase()])
				return;
		...
		{
			if (value == m_mode || !AxFlectrum[value.toUpperCase()])
				return;
