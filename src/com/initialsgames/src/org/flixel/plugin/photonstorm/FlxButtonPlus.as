 ...
 * 
 * v1.4 Added scrollFactor to button and swapped to using mouseInAxRect so buttons in scrolling worlds work
...
package org.flixel.plugin.photonstorm
{
	 ...
	 */
	public class AxButtonPlus extends AxGroup
	{
		...
		public var buttonNormal:AxExtendedSprite;
		public var buttonHighlight:AxExtendedSprite;
		
		public var textNormal:AxText;
		public var textHighlight:AxText;
		...
		/**
		 * Creates a new <code>AxButton</code> object with a gray background
		 ...
		 */
		public function AxButtonPlus(X:int, Y:int, Callback:Function, Params:Array = null, Label:String = null, Width:int = 100, Height:int = 20):void
		{
			super(4);
			...
			buttonNormal = new AxExtendedSprite(X, Y);
			buttonNormal.makeGraphic(Width, Height, borderColor);
			buttonNormal.stamp(AxGradient.createGradientAxSprite(Width - 2, Height - 2, offColor), 1, 1);
			buttonNormal.solid = false;
			buttonNormal.scrollFactor.x = 0;
			buttonNormal.scrollFactor.y = 0;
			
			buttonHighlight = new AxExtendedSprite(X, Y);
			buttonHighlight.makeGraphic(Width, Height, borderColor);
			buttonHighlight.stamp(AxGradient.createGradientAxSprite(Width - 2, Height - 2, onColor), 1, 1);
			...
			if (Label != null)
			{
				textNormal = new AxText(X, Y + 3, Width, Label);
				textNormal.setFormat(null, 8, 0xffffffff, "center", 0xff000000);
				
				textHighlight = new AxText(X, Y + 3, Width, Label);
		...
		override public function preUpdate():void
		{
			...
			if (!_initialized)
			{
				if(Ax.stage != null)
				{
					Ax.stage.addEventListener(MouseEvent.MOUSE_UP, onMouseUp);
					_initialized = true;
				}
			}
		}
		...
		/**
		 * If you wish to replace the two buttons (normal and hovered-over) with AxSprites, then pass them here.<br />
		 * Note: The pixel data is extract from the passed AxSprites and assigned locally, it doesn't actually use the sprites<br />
		 * or keep a reference to them.
		 * 
		 * @param	normal		The AxSprite to use when the button is in-active (not hovered over)
		 * @param	highlight	The AxSprite to use when the button is hovered-over by the mouse
		 */
		public function loadGraphic(normal:AxSprite, highlight:AxSprite):void
		{
		...
		protected function updateButton():void
		{
			...
			if (Ax.mouse.visible)
			{
				...
				{
					buttonNormal.cameras = Ax.cameras;
				}
				...
				while(i < l)
				{
					c = buttonNormal.cameras[i++] as AxCamera;
					
					if (AxMath.mouseInAxRect(false, buttonNormal.rect))
					{
						offAll = false;
		...
		override public function destroy():void
		{
			if (Ax.stage != null)
			{
				Ax.stage.removeEventListener(MouseEvent.MOUSE_UP, onMouseUp);
			}
		...
		/**
		 * Internal function for handling the actual callback call (for UI thread dependent calls like <code>AxU.openURL()</code>).
		 */
		protected function onMouseUp(event:MouseEvent):void
		{
			if (exists && visible && active && (_status == PRESSED) && (_onClick != null) && (pauseProof || !Ax.paused))
			{
		...
		/**
		 * Center this button (on the X axis) Uses Ax.width / 2 - button width / 2 to achieve this.<br />
		 * Doesn't take into consideration scrolling
		 */
		public function screenCenter():void
		{
			buttonNormal.x = (Ax.width / 2) - (width / 2);
			buttonHighlight.x = (Ax.width / 2) - (width / 2);
