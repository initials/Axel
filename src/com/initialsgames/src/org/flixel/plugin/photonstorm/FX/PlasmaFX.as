...
package org.flixel.plugin.photonstorm.FX 
{
	...
	/**
	 * Creates a plasma effect AxSprite
	 */
	...
	public class PlasmaFX extends BaseFX
	{
		...
		public function create(x:int, y:int, width:uint, height:uint, scaleX:uint = 1, scaleY:uint = 1):AxSprite
		{
			sprite = new AxSprite(x, y).makeGraphic(width, height, 0x0);
			
			if (scaleX != 1 || scaleY != 1)
			{
				sprite.scale = new AxPoint(scaleX, scaleY);
			...
			colours = AxColor.getHSVColorWheel();
			
			//colours = AxColor.getHSVColorWheel(140);	// now supports alpha :)
			//colours = AxGradient.createGradientArray(1, 360, [0xff000000, 0xff000000, 0xff000000, 0x00000000, 0xff000000], 2);	//	Lovely black reveal for over an image
			//colours = AxGradient.createGradientArray(1, 360, [0xff0000FF, 0xff000000, 0xff8F107C, 0xff00FFFF, 0xff0000FF], 1); // lovely purple black blue thingy
