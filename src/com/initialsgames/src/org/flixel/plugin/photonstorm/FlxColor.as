...
package org.flixel.plugin.photonstorm
{
	...
	/**
	 * <code>AxColor</code> is a set of fast color manipulation and color harmony methods.<br />
	 * Can be used for creating gradient maps or general color translation / conversion.
	 */
	public class AxColor
	{
		public function AxColor()
		{
		}
		...
		public static function getComplementHarmony(color:uint):uint
		{
			...
			var opposite:int = AxMath.wrapValue(hsv.hue, 180, 359);
		...
		public static function getAnalogousHarmony(color:uint, threshold:int = 30):Object
		{
			...
			{
				throw Error("AxColor Warning: Invalid threshold given to getAnalogousHarmony()");
			}
			
			var warmer:int = AxMath.wrapValue(hsv.hue, 359 - threshold, 359);
			var colder:int = AxMath.wrapValue(hsv.hue, threshold, 359);
		...
		public static function getSplitComplementHarmony(color:uint, threshold:int = 30):Object
		{
			...
			if (threshold >= 359 || threshold <= 0)
			{
				throw Error("AxColor Warning: Invalid threshold given to getSplitComplementHarmony()");
			}
			
			var opposite:int = AxMath.wrapValue(hsv.hue, 180, 359);
			
			var warmer:int = AxMath.wrapValue(hsv.hue, opposite - threshold, 359);
			var colder:int = AxMath.wrapValue(hsv.hue, opposite + threshold, 359);
			
			Ax.log("hue: " + hsv.hue + " opposite: " + opposite + " warmer: " + warmer + " colder: " + colder);
		...
		public static function getTriadicHarmony(color:uint):Object
		{
			...
			var triadic1:int = AxMath.wrapValue(hsv.hue, 120, 359);
			var triadic2:int = AxMath.wrapValue(triadic1, 120, 359);
		...
		public static function HSVtoRGB(h:Number, s:Number, v:Number, alpha:uint = 255):uint
		{
			...
			else
			{
				...
				switch (int(h))
				{
					...
					default:
						Ax.log("AxColor Error: HSVtoRGB : Unknown color");
				}
			}
		...
		public static function getRandomColor(min:uint = 0, max:uint = 255, alpha:uint = 255):uint
		{
			...
			if (max > 255)
			{
				Ax.log("AxColor Warning: getRandomColor - max value too high");
				return getColor24(255, 255, 255);
			}
			
			if (min > max)
			{
				Ax.log("AxColor Warning: getRandomColor - min value higher than max");
				return getColor24(255, 255, 255);
			}
