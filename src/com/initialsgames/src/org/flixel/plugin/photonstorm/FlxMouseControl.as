...
package org.flixel.plugin.photonstorm 
{
	...
	public class AxMouseControl extends AxBasic
	{
		/**
		...
		/**
		 * The value that the AxExtendedSprites are sorted by before deciding which is "on-top" for click select
		 */
		...
		/**
		 * The AxExtendedSprite that is currently being dragged, if any.
		 */
		public static var dragTarget:AxExtendedSprite;
		
		/**
		 * The AxExtendedSprite that currently has the mouse button pressed on it
		 */
		public static var clickTarget:AxExtendedSprite;
		private static var clickStack:Array = new Array;
		private static var clickCoords:AxPoint;
		...
		/**
		 * The mouse can be set to only be active within a specific AxRect region of the game world.
		 * If outside this AxRect no clicks, drags or throws will be processed.
		 ...
		 * Set the AxRect to null to disable the zone.
		 */
		public static var mouseZone:AxRect;
		
		/**
		 * Instead of using a mouseZone (which is calculated in world coordinates) you can limit the mouse to the Ax.camera.deadzone area instead.
		...
		public function AxMouseControl() 
		{
		}
		
		/**
		 * Adds the given AxExtendedSprite to the stack of potential sprites that were clicked, the stack is then sorted and the final sprite is selected from that
		 * 
		 * @param	item	The AxExtendedSprite that was clicked by the mouse
		 */
		public static function addToStack(item:AxExtendedSprite):void
		{
			if (mouseZone is AxRect)
			{
				if (AxMath.pointInAxRect(Ax.mouse.x, Ax.mouse.y, mouseZone) == true)
				{
		...
		/**
		 * Main Update Loop - checks mouse status and updates AxExtendedSprites accordingly
		 */
		override public function update():void
		{
			//	Update mouse speed
			speedX = Ax.mouse.screenX - oldX;
			speedY = Ax.mouse.screenY - oldY;
			
			oldX = Ax.mouse.screenX;
			oldY = Ax.mouse.screenY;
			...
			if (hasClickTarget)
			{
				if (Ax.mouse.pressed())
				{
					...
					if (isDragging == false && clickTarget.draggable && (clickCoords.x != Ax.mouse.x || clickCoords.y != Ax.mouse.y))
					{
						//	Drag on
						isDragging = true;
				...
				if (linkToDeadZone)
				{
					if (AxMath.mouseInAxRect(false, Ax.camera.deadzone) == false)
					{
						releaseMouse();
					}
				}
				else if (AxMath.mouseInAxRect(true, mouseZone) == false)
				{
			...
			else
			{
				...
				//	If you are wondering how the brand new array can have anything in it by now, it's because AxExtendedSprite
				//	adds itself to the clickStack
				
				if (Ax.mouse.pressed() && clickStack.length > 0)
				{
					assignClickedSprite();
				}
			}
		 ...
		 */
		private function sortHandler(item1:AxExtendedSprite, item2:AxExtendedSprite):int
		{
