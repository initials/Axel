 ...
 * v1.3 Renamed from AxHealthBar and made less specific / far more flexible
...
package org.flixel.plugin.photonstorm 
{
	...
	/**
	 * AxBar is a quick and easy way to create a graphical bar which can
	...
	public class AxBar extends AxSprite
	{
		...
		/**
		 * fixedPosition controls if the AxBar sprite is at a fixed location on screen, or tracking its parent
		 */
		public var fixedPosition:Boolean = true;
		
		/**
		 * The positionOffset controls how far offset the AxBar is from the parent sprite (if at all)
		 */
		public var positionOffset:AxPoint;
		 ...
		 * @param	direction 	One of the AxBar.FILL_ constants (such as FILL_LEFT_TO_RIGHT, FILL_TOP_TO_BOTTOM etc)
		 ...
		 * @param	variable	The variable of the object that is used to determine the bar position. For example if the parent was an AxSprite this could be "health" to track the health value
		 ...
		 */
		public function AxBar(x:int, y:int, direction:uint = FILL_LEFT_TO_RIGHT, width:int = 100, height:int = 10, parentRef:* = null, variable:String = "", min:Number = 0, max:Number = 100, border:Boolean = false):void
		{
			super(x, y);
		...
		/**
		 * Track the parent AxSprites x/y coordinates. For example if you wanted your sprite to have a floating health-bar above their head.<br />
		...
		public function trackParent(offsetX:int, offsetY:int):void
		{
			...
			positionOffset = new AxPoint(offsetX, offsetY);
		...
		/**
		 * Sets a parent for this AxBar. Instantly replaces any previously set parent and refreshes the bar.
		 * 
		 * @param	parentRef	A reference to an object in your game that you wish the bar to track
		 * @param	variable	The variable of the object that is used to determine the bar position. For example if the parent was an AxSprite this could be "health" to track the health value
		 * @param	track		If you wish the AxBar to track the x/y coordinates of parent set to true (default false)
		 * @param	offsetX		The offset on X in relation to the origin x/y of the parent
		 * @param	offsetY		The offset on Y in relation to the origin x/y of the parent
		 */
		...
		/**
		 * Sets callbacks which will be triggered when the value of this AxBar reaches min or max.<br>
		 * Functions will only be called once and not again until the value changes.<br>
		 * Optionally the AxBar can be killed if it reaches min, but if will fire the empty callback first (if set)
		 * 
		 * @param	onEmpty			The function that is called if the value of this AxBar reaches min
		 * @param	onFilled		The function that is called if the value of this AxBar reaches max
		 * @param	killOnEmpty		If set it will call AxBar.kill() if the value reaches min
		 */
		...
		/**
		 * If this AxBar should be killed when its value reaches empty, set to true
		 */
		...
		/**
		 * Set the minimum and maximum allowed values for the AxBar
		 * 
		 * @param	min			The minimum value. I.e. for a progress bar this would be zero (nothing loaded yet)
		 * @param	max			The maximum value the bar can reach. I.e. for a progress bar this would typically be 100.
		 */
		public function setRange(min:Number, max:Number):void
		{
			if (max <= min)
			{
				throw Error("AxBar: max cannot be less than or equal to min");
				return;
			}
		...
		public function createGradientBar(empty:Array, fill:Array, chunkSize:int = 1, rotation:int = 180, showBorder:Boolean = false, border:uint = 0xffffffff):void
		{
			...
			{
				emptyBar = new BitmapData(barWidth, barHeight, true, border);
				AxGradient.overlayGradientOnBitmapData(emptyBar, barWidth - 2, barHeight - 2, empty, 1, 1, chunkSize, rotation);
				
				filledBar = new BitmapData(barWidth, barHeight, true, border);
				AxGradient.overlayGradientOnBitmapData(filledBar, barWidth - 2, barHeight - 2, fill, 1, 1, chunkSize, rotation);
			}
			else
			{
				emptyBar = AxGradient.createGradientBitmapData(barWidth, barHeight, empty, chunkSize, rotation);
				filledBar = AxGradient.createGradientBitmapData(barWidth, barHeight, fill, chunkSize, rotation);
			}
