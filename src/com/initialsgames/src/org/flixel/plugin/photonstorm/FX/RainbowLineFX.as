 ...
 * 
 * v1.0 Built into the new AxSpecialFX system
 * 
 * @version 1.0 - May 9th 2011
 * @link http://www.photonstorm.com
 * @author Richard Davey / Photon Storm
 * @see Requires AxGradient, AxMath
*/
...
package org.flixel.plugin.photonstorm.FX 
{
	...
	public class RainbowLineFX extends BaseFX
	{
		...
		/**
		 * Creates a Color Line AxSprite.
		 * 
		 * @param	x			The x coordinate of the AxSprite in game world pixels
		 * @param	y			The y coordinate of the AxSprite in game world pixels
		 * @param	width		The width of the AxSprite in pixels
		 * @param	height		The height of the AxSprite in pixels
		 ...
		 * 
		 * @return	An AxSprite which automatically updates each draw() to cycle the colors through it
		 */
		public function create(x:int, y:int, width:uint, height:uint = 1, colors:Array = null, colorWidth:uint = 360, colorSpeed:uint = 1, stepSize:uint = 1, fadeWidth:uint = 128, fadeColor:uint = 0xff000000):AxSprite
		{
			sprite = new AxSprite(x, y).makeGraphic(width, height, 0x0);
			
			canvas = new BitmapData(width, height, true, 0x0);
			
			if (colors is Array)
			{
				lineColors = AxGradient.createGradientArray(1, colorWidth, colors);
			}
			else
			{
				lineColors = AxColor.getHSVColorWheel();
			}
			...
			if (fadeWidth != 0)
			{
				var blackToFirst:Array = AxGradient.createGradientArray(1, fadeWidth, [ fadeColor, fadeColor, fadeColor, lineColors[0] ]);
				var lastToBlack:Array = AxGradient.createGradientArray(1, fadeWidth, [ lineColors[maxColor], fadeColor, fadeColor, fadeColor, fadeColor ]);
		...
		public function updateColors(colors:Array, colorWidth:uint = 360, resetCurrentColor:Boolean = false, fadeWidth:uint = 128, fadeColor:uint = 0xff000000):void
		{
			...
			if (fadeWidth != 0)
			{
				var blackToFirst:Array = AxGradient.createGradientArray(1, fadeWidth, [ 0xff000000, 0xff000000, 0xff000000, lineColors[0] ]);
				var lastToBlack:Array = AxGradient.createGradientArray(1, fadeWidth, [ lineColors[maxColor], 0xff000000, 0xff000000, 0xff000000, 0xff000000 ]);
		...
		/**
		 * Doesn't need to be called directly as it's called by the AxSpecialFX Plugin.<br>
		 * Set active to false if you wish to disable the effect.<br>
		 * Pass the effect to AxSpecialFX.erase() if you wish to destroy this effect.
		 */
		public function draw():void
		{
			...
			{
				var c:int = AxMath.wrapValue(currentColor + x, 1, maxColor);
