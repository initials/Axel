package org.flixel
{
	 ...
	 * @author	Adam Atomic
	 */
	public class AxSound extends AxBasic
	{
		/**
		...
		/**
		 * The AxSound constructor gets all the variables initialized, but NOT ready to play a sound yet.
		 */
		public function AxSound()
		{
			super();
			createSound();
		}
		...
		override public function update():void
		{
			...
			if(_target != null)
			{
				radial = AxU.getDistance(new AxPoint(_target.x,_target.y),new AxPoint(x,y))/_radius;
		 ...
		 * @param	AutoDestroy		Whether or not this <code>AxSound</code> instance should be destroyed when the sound finishes playing.  Default value is false, but Ax.play() and Ax.stream() will set it to true by default.
		 * 
		 * @return	This <code>AxSound</code> instance (nice for chaining stuff together, if you're into that).
		 */
		public function loadEmbedded(EmbeddedSound:Class, Looped:Boolean=false, AutoDestroy:Boolean=false):AxSound
		{
			stop();
			createSound();
		 ...
		 * @return	This <code>AxSound</code> instance (nice for chaining stuff together, if you're into that).
		 */
		public function loadStream(SoundURL:String, Looped:Boolean=false, AutoDestroy:Boolean=false):AxSound
		{
			stop();
			createSound();
			_sound = new Sound();
		 ...
		 * based on distance from a particular AxCore object.
		 * 
		 ...
		 * 
		 * @return	This AxSound instance (nice for chaining stuff together, if you're into that).
		 */
		public function proximity(X:Number,Y:Number,Object:AxEntity,Radius:Number,Pan:Boolean=true):AxSound
		{
			x = X;
			y = Y;
			_target = Object;
		...
		{
			_transform.volume = (Ax.mute?0:1)*Ax.volume*_volume*_volumeAdjust;
			if(_channel != null)
