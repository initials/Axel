package org.flixel
{
	...
	/**
	 * <code>AxEmitter</code> is a lightweight particle emitter.
	 * It can be used for one-time explosions or for
	 * continuous fx like rain and fire.  <code>AxEmitter</code>
	 * is not optimized or anything; all it does is launch
	 * <code>AxParticle</code> objects out at set intervals
	 * by setting their positions and velocities accordingly.
	 * It is easy to use and relatively efficient,
	 * relying on <code>AxGroup</code>'s RECYCLE POWERS.
	 * 
	 * @author	Adam Atomic
	 */
	public class AxEmitter extends AxGroup
	{
		/**
		 ...
		 * Default is <code>AxParticle</code>.
		 */
		...
		/**
		 * Creates a new <code>AxEmitter</code> object at a specific position.
		 ...
		 */
		public function AxEmitter(X:Number=0, Y:Number=0, Size:Number=0)
		{
			super(Size);
			x = X;
			y = Y;
		 ...
		 * 
		 * @param	Graphics		If you opted to not pre-configure an array of AxSprite objects, you can simply pass in a particle image or sprite sheet.
		 * @param	Quantity		The number of particles to generate when using the "create from image" option.
		 * @param	BakedRotations	How many frames of baked rotation to use (boosts performance).  Set to zero to not use baked rotations.
		 * @param	Multiple		Whether the image in the Graphics param is a single particle or a bunch of particles (if it's a bunch, they need to be square!).
		 * @param	Collide			Whether the particles should be flagged as not 'dead' (non-colliding particles are higher performance).  0 means no collisions, 0-1 controls scale of particle's bounding box.
		 * 
		 * @return	This AxEmitter instance (nice for chaining stuff together, if you're into that).
		 */
		public function makeParticles(Graphics:Class, Quantity:uint=50, BakedRotations:uint=16, Multiple:Boolean=false, Collide:Number=0.8):AxEmitter
		{
			maxSize = Quantity;
			...
			if(Multiple)
			{ 
				var sprite:AxSprite = new AxSprite();
			...
			var particle:AxParticle;
			var i:uint = 0;
			while(i < Quantity)
			{
				if(particleClass == null)
					particle = new AxParticle();
				else
					particle = new particleClass();
				if(Multiple)
				{
					randomFrame = Ax.random()*totalFrames;
					if(BakedRotations > 0)
				...
				if(Collide > 0)
				{
					...
					particle.allowCollisions = AxEntity.NONE;
		...
		{
			var particle:AxParticle = recycle(AxParticle) as AxParticle;
			particle.lifespan = lifespan;
			particle.elasticity = bounce;
			particle.reset(x - (particle.width>>1) + Ax.random()*width, y - (particle.height>>1) + Ax.random()*height);
			particle.visible = true;
			
			if(minParticleSpeed.x != maxParticleSpeed.x)
				particle.velocity.x = minParticleSpeed.x + Ax.random()*(maxParticleSpeed.x-minParticleSpeed.x);
			else
				particle.velocity.x = minParticleSpeed.x;
			if(minParticleSpeed.y != maxParticleSpeed.y)
				particle.velocity.y = minParticleSpeed.y + Ax.random()*(maxParticleSpeed.y-minParticleSpeed.y);
			else
				particle.velocity.y = minParticleSpeed.y;
			particle.acceleration.y = gravity;
			
			if(minRotation != maxRotation)
				particle.angularVelocity = minRotation + Ax.random()*(maxRotation-minRotation);
			else
				particle.angularVelocity = minRotation;
			if(particle.angularVelocity != 0)
				particle.angle = Ax.random()*360-180;
		...
		/**
		 * Change the emitter's midpoint to match the midpoint of a <code>AxEntity</code>.
		 * 
		 * @param	Object		The <code>AxEntity</code> that you want to sync up with.
		 */
		public function at(Object:AxEntity):void
		{
