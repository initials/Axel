package org.flixel
{
	 ...
	 * You can add more cameras or even replace the main camera using utilities in <code>AxG</code>.
	 * 
	 * @author Adam Atomic
	 */
	public class AxCamera extends AxBasic
	{
		/**
		...
		/**
		 * Tells the camera to follow this <code>AxEntity</code> object around.
		 */
		...
		/**
		 * The natural background color of the camera. Defaults to Ax.bgColor.
		 * NOTE: can be transparent for crazy FX!
		 */
		public var bgColor:uint;
		/**
		 * Sometimes it's easier to just work with a <code>AxSprite</code> than it is to work
		 * directly with the <code>BitmapData</code> buffer.  This sprite reference will
		 * allow you to do exactly that.
		 */
		public var screen:AxSprite;
		 ...
		 * Internal, used to control the "shake" special effect.
		 */
		protected var _fxShakeOffset:AxPoint;
		/**
		 * Internal, used to control the "shake" special effect.
		 */
		 ...
		 */
		public function AxCamera(X:int,Y:int,Width:int,Height:int,Zoom:Number=0)
		{
			x = X;
			y = Y;
			width = Width;
		...
		override public function update():void
		{
			...
			{
				_fxFlashAlpha -= Ax.elapsed/_fxFlashDuration;
			...
			{
				_fxFadeAlpha += Ax.elapsed/_fxFadeDuration;
				if(_fxFadeAlpha >= 1.0)
				{
					_fxFadeAlpha = 1.0;
			...
			if(_fxShakeDuration > 0)
			{
				...
				else
				{
					if((_fxShakeDirection == SHAKE_BOTH_AXES) || (_fxShakeDirection == SHAKE_HORIZONTAL_ONLY))
						_fxShakeOffset.x = (Ax.random()*_fxShakeIntensity*width*2-_fxShakeIntensity*width)*_zoom;
					if((_fxShakeDirection == SHAKE_BOTH_AXES) || (_fxShakeDirection == SHAKE_VERTICAL_ONLY))
						_fxShakeOffset.y = (Ax.random()*_fxShakeIntensity*height*2-_fxShakeIntensity*height)*_zoom;
				}
			}
		}
		
		/**
		 * Tells this camera object what <code>AxEntity</code> to track.
		 * 
		 ...
		 */
		public function follow(Target:AxEntity, Style:uint=STYLE_LOCKON):void
		{
			target = Target;
			var helper:Number;
			switch(Style)
			{
				case STYLE_PLATFORMER:
					...
					deadzone = new AxRect((width-w)/2,(height-h)/2 - h*0.25,w,h);
					break;
				case STYLE_TOPDOWN:
					helper = AxU.max(width,height)/4;
					deadzone = new AxRect((width-helper)/2,(height-helper)/2,helper,helper);
					break;
				case STYLE_TOPDOWN_TIGHT:
					helper = AxU.max(width,height)/8;
					deadzone = new AxRect((width-helper)/2,(height-helper)/2,helper,helper);
					break;
				case STYLE_LOCKON:
				default:
		 ...
		 */
		public function focusOn(Point:AxPoint):void
		{
		...
		public function setBounds(X:Number=0, Y:Number=0, Width:Number=0, Height:Number=0, UpdateWorld:Boolean=false):void
		{
			...
			if(UpdateWorld)
				Ax.worldBounds.copyFrom(bounds);
			update();
		}
		 ...
		 * 
		 * @return	A reference to this <code>AxCamera</code> object.
		 */
		public function copyFrom(Camera:AxCamera):AxCamera
		{
		 ...
		 * since cameras aren't nested in an extra display object yet.
		 */
		public function getScale():AxPoint
		{
		 ...
		 * using the existing <code>AxCamera</code> variables.
		 * 
