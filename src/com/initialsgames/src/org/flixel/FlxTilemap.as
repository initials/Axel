package org.flixel
{
	 ...
	 * @author	Adam Atomic
	 */
	public class AxTilemap extends AxEntity
	{
		...
		public function AxTilemap()
		{
			...
			_debugRect = null;
			_lastVisualDebug = Ax.visualDebug;
			_startingIndex = 0;
		}
		...
		override public function destroy():void
		{
			...
			while(i < l)
				(_tileObjects[i++] as AxTile).destroy();
			_tileObjects = null;
			i = 0;
			l = _buffers.length;
			while(i < l)
				(_buffers[i++] as AxTilemapBuffer).destroy();
			_buffers = null;
			_data = null;
			_rects = null;
			_debugTileNotSolid = null;
		 ...
		 * 
		 * @return	A pointer this instance of AxTilemap, for chaining as usual :)
		 */
		public function loadMap(MapData:String, TileGraphic:Class, TileWidth:uint=0, TileHeight:uint=0, AutoTile:uint=OFF, StartingIndex:uint=0, DrawIndex:uint=1, CollideIndex:uint=1):AxTilemap
		{
			auto = AutoTile;
		...
		protected function makeDebugTile(Color:uint):BitmapData
		{
			...
			debugTile.draw(Ax.flashGfxSprite);
			return debugTile;
		}
		...
		{
			if(_lastVisualDebug != Ax.visualDebug)
			{
				_lastVisualDebug = Ax.visualDebug;
				setDirty();
			}
		 ...
		 * 
		 * @param	Buffer		The <code>AxTilemapBuffer</code> you are rendering to.
		 * @param	Camera		The related <code>AxCamera</code>, mainly for scroll values.
		 */
		protected function drawTilemap(Buffer:AxTilemapBuffer,Camera:AxCamera):void
		{
			Buffer.fill();
		...
		override public function draw():void
		{
			...
			while(i < l)
			{
				camera = cameras[i];
				if(_buffers[i] == null)
					_buffers[i] = new AxTilemapBuffer(_tileWidth,_tileHeight,widthInTiles,heightInTiles,camera);
				buffer = _buffers[i++] as AxTilemapBuffer;
				if(!buffer.dirty)
				{
		...
		public function getData(Simple:Boolean=false):Array
		{
			...
			while(i < l)
			{
				data[i] = ((_tileObjects[_data[i]] as AxTile).allowCollisions > 0)?1:0;
				i++;
			}
			return data;
		}
		...
		public function setDirty(Dirty:Boolean=true):void
		{
			...
			while(i < l)
				(_buffers[i++] as AxTilemapBuffer).dirty = Dirty;
		}
		 ...
		 * 
		 * @return	A <code>AxPath</code> from the start to the end.  If no path could be found, then a null reference is returned.
		 */
		public function findPath(Start:AxPoint,End:AxPoint,Simplify:Boolean=true,RaySimplify:Boolean=false):AxPath
		{
			...
			if( ((_tileObjects[_data[startIndex]] as AxTile).allowCollisions > 0) ||
				((_tileObjects[_data[endIndex]] as AxTile).allowCollisions > 0) )
				return null;
			...
			var node:AxPoint;
			node = points[points.length-1] as AxPoint;
			node.x = Start.x;
			node.y = Start.y;
			node = points[0] as AxPoint;
			node.x = End.x;
			node.y = End.y;
		 ...
		 * 
		 * @return	A Flash <code>Array</code> of <code>AxPoint</code> nodes.  If the end tile could not be found, then a null <code>Array</code> is returned instead.
		 */
		protected function computePathDistance(StartIndex:uint, EndIndex:uint):Array
		{
			...
			while(i < mapSize)
			{
				if((_tileObjects[_data[i]] as AxTile).allowCollisions)
					distances[i] = -2;
				else
					distances[i] = -1;
				i++;
		 ...
		 * @param	Points	A Flash <code>Array</code> of <code>AxPoint</code> nodes composing the path from the start to the end, compiled in reverse order.
		 */
		protected function walkPath(Data:Array,Start:uint,Points:Array):void
		{
			Points.push(new AxPoint(x + uint(Start%widthInTiles)*_tileWidth + _tileWidth*0.5, y + uint(Start/widthInTiles)*_tileHeight + _tileHeight*0.5));
			if(Data[Start] == 0)
				return;
		...
		/**
		 * Checks to see if some <code>AxEntity</code> overlaps this <code>AxEntity</code> object in world space.
		 * If the group has a LOT of things in it, it might be faster to use <code>Ax.overlaps()</code>.
		 ...
		 */
		override public function overlaps(ObjectOrGroup:AxBasic,InScreenSpace:Boolean=false,Camera:AxCamera=null):Boolean
		{
			if(ObjectOrGroup is AxGroup)
			{
				var results:Boolean = false;
				var basic:AxBasic;
				var i:uint = 0;
				var members:Array = (ObjectOrGroup as AxGroup).members;
				while(i < length)
				{
					basic = members[i++] as AxBasic;
					if(basic is AxEntity)
					{
						if(overlapsWithCallback(basic as AxEntity))
							results = true;
					}
					else
					{
			...
			else if(ObjectOrGroup is AxEntity)
				return overlapsWithCallback(ObjectOrGroup as AxEntity);
			return false;
		}
		
		/**
		 * Checks to see if this <code>AxEntity</code> were located at the given position, would it overlap the <code>AxEntity</code> or <code>AxGroup</code>?
		 ...
		 * @return	Whether or not the two objects overlap.
		 */
		override public function overlapsAt(X:Number,Y:Number,ObjectOrGroup:AxBasic,InScreenSpace:Boolean=false,Camera:AxCamera=null):Boolean
		{
			if(ObjectOrGroup is AxGroup)
			{
				var results:Boolean = false;
				...
				while(i < length)
				{
					...
					if(basic is AxEntity)
					{
						_point.x = X;
						_point.y = Y;
						if(overlapsWithCallback(basic as AxEntity,null,false,_point))
							results = true;
					}
					else
					{
			...
			else if(ObjectOrGroup is AxEntity)
			{
				_point.x = X;
				_point.y = Y;
				return overlapsWithCallback(ObjectOrGroup as AxEntity,null,false,_point);
			}
			return false;
		}
		 ...
		 * 
		 * @param	Object				The <code>AxEntity</code> you are checking for overlaps against.
		 * @param	Callback			An optional function that takes the form "myCallback(Object1:AxEntity,Object2:AxEntity)", where Object1 is a AxTile object, and Object2 is the object passed in in the first parameter of this method.
		 * @param	FlipCallbackParams	Used to preserve A-B list ordering from AxEntity.separate() - returns the AxTile object as the second parameter instead.
		 ...
		 */
		public function overlapsWithCallback(Object:AxEntity,Callback:Function=null,FlipCallbackParams:Boolean=false,Position:AxPoint=null):Boolean
		{
			var results:Boolean = false;
			...
			var selectionX:int = AxU.floor((Object.x - X)/_tileWidth);
			var selectionY:int = AxU.floor((Object.y - Y)/_tileHeight);
			var selectionWidth:uint = selectionX + (AxU.ceil(Object.width/_tileWidth)) + 1;
			var selectionHeight:uint = selectionY + AxU.ceil(Object.height/_tileHeight) + 1;
			...
			//Then loop through this selection of tiles and call AxEntity.separate() accordingly
			...
			while(row < selectionHeight)
			{
				...
				{
					overlapFound = false;
					tile = _tileObjects[_data[rowStart+column]] as AxTile;
		...
		/**
		 * Checks to see if a point in 2D world space overlaps this <code>AxEntity</code> object.
		 * 
		 ...
		 */
		override public function overlapsPoint(Point:AxPoint,InScreenSpace:Boolean=false,Camera:AxCamera=null):Boolean
		{
			if(!InScreenSpace)
				return (_tileObjects[_data[uint(uint((Point.y-y)/_tileHeight)*widthInTiles + (Point.x-x)/_tileWidth)]] as AxTile).allowCollisions > 0;
			...
			return (_tileObjects[_data[uint(uint((Point.y-_point.y)/_tileHeight)*widthInTiles + (Point.x-_point.x)/_tileWidth)]] as AxTile).allowCollisions > 0;
		}
		...
		public function getTileCoords(Index:uint,Midpoint:Boolean=true):Array
		{
			...
			while(i < l)
			{
				if(_data[i] == Index)
				{
					point = new AxPoint(x + uint(i%widthInTiles)*_tileWidth,y + uint(i/widthInTiles)*_tileHeight);
					if(Midpoint)
					{
		 ...
		 * @param	AllowCollisions	Modify the tile or tiles to only allow collisions from certain directions, use AxEntity constants NONE, ANY, LEFT, RIGHT, etc.  Default is "ANY".
		 * @param	Callback		The function to trigger, e.g. <code>lavaCallback(Tile:AxTile, Object:AxEntity)</code>.
		 ...
		 */
		public function follow(Camera:AxCamera=null,Border:int=0,UpdateWorld:Boolean=true):void
		{
			if(Camera == null)
				Camera = Ax.camera;
		...
		/**
		 * Get the world coordinates and size of the entire tilemap as a <code>AxRect</code>.
		 * 
		 * @param	Bounds		Optional, pass in a pre-existing <code>AxRect</code> to prevent instantiation of a new object.
		 * 
		 * @return	A <code>AxRect</code> containing the world coordinates and size of the entire tilemap.
		 */
		public function getBounds(Bounds:AxRect=null):AxRect
		{
			if(Bounds == null)
				Bounds = new AxRect();
		 ...
		 */
		public function ray(Start:AxPoint, End:AxPoint, Result:AxPoint=null, Resolution:Number=1):Boolean
		{
			...
			while(i < steps)
			{
				...
				if((_tileObjects[_data[tileY*widthInTiles+tileX]] as AxTile).allowCollisions)
				{
		 ...
		 * 
		 * @return	A comma-separated string containing the level data in a <code>AxTilemap</code>-friendly format.
		 */
		...
		{
			var tile:AxTile = _tileObjects[_data[Index]] as AxTile;
